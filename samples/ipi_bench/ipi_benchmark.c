...
static int ipi_bench_single_task(void *data)
{
	...
	if (atomic64_read(&ready_to_run) != atomic64_read(&should_run)) {
		pr_info("ipi_bench: BUG, exit benchmark\n");
		return -1;
	}
...
static int ipi_bench_one_task(struct bench_args *ba)
{
	...
	pr_info("ipi_bench: prepare single IPI from CPU[%3d] to CPU[%3d]\n", src, dst);
	...
	if (IS_ERR(tsk)) {
		pr_info("ipi_bench: create kthread failed\n");
		return -1;
	}
...
static int ipi_bench_many(void)
{
	...
	for (loop = loops; loop > 0; loop--) {
		if (lock)
			smp_call_function_many(cpu_online_mask, ipi_bench_spinlock, &spinlock,
									 wait);
		else
...
static int ipi_bench_all_task(struct bench_args *ba)
{
	...
	pr_info("ipi_bench: prepare broadcast IPI from CPU[%3d] to all CPUs\n", src);
...
static inline void ipi_bench_report_single(struct bench_args *ba)
{
	...
	if (!finish) {
		pr_info("ipi_bench: too many loops\n");
		return;
	}
...
static int ipi_bench_mesh(int pairs, int acrossnuma)
{
	...
	for (i = 0; i < pairs; i++) {
		...
		if ((ba->src < 0) || (ba->dst < 0)) {
			pr_info("ipi_bench: init mesh failed\n");
			goto out;
		}
	}
	...
	pr_info("ipi_bench: throughput %ld ipi/s\n",
...
static int ipi_bench_all(int broadcasts)
{
	...
	for (i = 0; i < broadcasts; i++) {
		...
		ba->src = __random_unused_cpu_in_cpumask(cpumask, -1);
		if (ba->src < 0) {
			pr_info("ipi_bench: init broadcast workers failed\n");
			goto out;
		}
	}
...
static void ipi_bench_options(void)
{
	...
	pr_info("ipi_bench: you should run insmod ipi_bench.ko options=XX, bit flags:\n");
	for (i = 0; i < ARRAY_SIZE(benchcases); i++)
		pr_info("ipi_bench:\tbit[%d] %s\n", i, benchcases[i]);
}
...
static int ipi_bench_init_params(void)
{
	...
	if (num_cpus < 2) {
		pr_info("ipi_bench: total cpu num %d, no need to test\n", num_cpus);
		return -1;
	}

	if ((srccpu >= num_cpus) || (dstcpu >= num_cpus)) {
		pr_info("ipi_bench: cpu out of range, total cpu num %d\n", num_cpus);
		return -1;
	}

	if (pairs > (num_cpus / 2)) {
		pr_info("ipi_bench: pairs out of range, total cpu num %d\n", num_cpus);
		return -1;
	}
