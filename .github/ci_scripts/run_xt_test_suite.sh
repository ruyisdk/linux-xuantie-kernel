...
#
# Usage:
#   .github/ci_scripts/run_xt_test_suite.sh \
...
if [[ -d "$RUNNERS_SRC" ]]; then
    if [[ ! -d "$RUNNERS_DST" ]] || [[ "$RUNNERS_SRC" != "$RUNNERS_DST" ]]; then
        cp -a "$RUNNERS_SRC" "$RUNNERS_DST" 2>/dev/null || true
    fi
...
"$CHECK_QEMU" 'modprobe virtio_net 2>/dev/null || true; ip link set eth0 up && udhcpc -i eth0 -t 5 -n || dhclient eth0 || true; ip a' 90 || true
...
"$CHECK_QEMU" 'mkdir -p /etc/init.ci/kernel_testsuite && cp -a /mnt/hostshare/kernel_testsuite/* /etc/init.ci/kernel_testsuite/ && chmod +x /etc/init.ci/kernel_testsuite/* && ls /etc/init.ci/kernel_testsuite/ | wc -l' 120 \
    || fail "failed to provision runner scripts from 9p"

# ---- test cases ------------------------------------------------------------
# Format per row (tab-separated):
#   id  apt_pkgs  run_cmd  timeout_sec  pass_re  fail_re
read -r -d '' CASE_TABLE <<'CASEEOF' || true
kselftest-cgroup	perl* kernel-selftest util-linux	/etc/init.ci/kernel_testsuite/kselftest_run CGROUP	7200	KSELFTEST TEST cgroup PASSED	ERROR!
kselftest-fs	perl* kernel-selftest util-linux-taskset	/etc/init.ci/kernel_testsuite/kselftest_run FS	7200	KSELFTEST TEST filesystems PASSED	ERROR!
kselftest-ftrace	perl* kernel-selftest util-linux-taskset	/etc/init.ci/kernel_testsuite/kselftest_run FTRACE	7200	KSELFTEST TEST ftrace PASSED	ERROR!
kselftest-hotplug	perl* kernel-selftest util-linux-taskset	/etc/init.ci/kernel_testsuite/kselftest_run HOTPLUG	7200	KSELFTEST TEST hotplug PASSED	ERROR!
kselftest-mm	perl* kernel-selftest util-linux-taskset	/etc/init.ci/kernel_testsuite/kselftest_run MM	7200	KSELFTEST TEST mm PASSED	ERROR!
