...
static inline struct k230_peridma_desc *
vd_to_k230_peridma_desc(struct virt_dma_desc *vd)
{
			   ...
			   unsigned long flags, void *context)
{
	...
	dev_vdbg(vchan2dev(vchan),
		 ": prep_slave_sg: sgl: %px sglen: %ud flags: %#lx", sgl, sglen,
		 flags);
	...
	dev_vdbg(vchan2dev(vchan),
		 "dma_async_tx_descriptor: prepare desc(%px) \n", desc);
...
static int k230_peridma_terminate_all(struct dma_chan *dchan)
{
	...
	if (vchan->pchan) {
		dev_vdbg(vchan2dev(vchan),
			 "k230_peridma_terminate_all: release pchan %x \n",
			 vchan->pchan->id);
		...
		dev_vdbg(vchan2dev(vchan),
			 "k230_peridma_terminate_all: vchan->pchan is NULL \n");
...
static void peridma_vchan_start_first_queued(struct k230_peridma_vchan *vchan)
{
	...
	if (!vd) {
		dev_vdbg(
			vchan2dev(vchan),
			"peridma_vchan_start_first_queued: no next vd found \n");
		goto freepchan;
	}

	desc = vd_to_k230_peridma_desc(vd);
	dev_vdbg(vchan2dev(vchan),
		 "peridma_vchan_start_first_queued: desc %px \n", desc);
...
static int k230_peridma_vchan_pause(struct dma_chan *dchan)
{
	...
	if (!vchan->pchan) {
		dev_err(vchan2dev(vchan),
			"k230_peridma_vchan_pause: vchan->pchan is NULL \n");
		dump_stack();
		return -EBUSY;
	}
					   ...
					   struct of_dma *ofdma)
{
	...
	dev_vdbg(
		priv->dev,
		"k230_pdma_of_xlate: priority=%x, dev_tout=%x, dat_endian=%x, dev_sel=%x \n",
...
static int k230_peridma_probe(struct platform_device *pdev)
{
	...
	dev_vdbg(
		&pdev->dev,
		"Successfully probed K230 peridma controller, register mapped at %pllx(PHY %llu) \n",
		priv->base, res->start);
