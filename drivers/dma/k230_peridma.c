...
struct k230_peridma_hwdesc {
	struct usr_pdma_cfg_t usr_pdma_chn_cfg;
	bool last;
...
struct k230_peridma_vchan {
	...
	u32 priority;
	u32 dev_tout;
	enum ch_dat_endian_t dat_endian;
...
struct pdma_chn_llt_t {
	dma_addr_t llt_list_p;
	struct pdma_llt_t *llt_list_v;
	bool use;
};
static struct pdma_chn_llt_t g_pdma_llt_list[CH_NUM];

static void free_pchan(struct k230_peridma_dev *pdev,
		       struct k230_peridma_pchan *pchan);
...
static inline struct k230_peridma_vchan *
vc_to_k230_peridma_vchan(struct virt_dma_chan *vc)
{
	return container_of(vc, struct k230_peridma_vchan, vchan);
}

static inline struct k230_peridma_vchan *
dchan_to_k230_peridma_vchan(struct dma_chan *dchan)
{
...
static void peridma_desc_put(struct k230_peridma_desc *desc)
{
	...
	list_for_each_entry_safe(hwdesc, tmp, &desc->xfer_list, list)
		kfree(hwdesc);
	if (!desc->cyclic) {
		list_for_each_entry_safe(hwdesc, tmp, &desc->completed_list,
					 list)
			kfree(hwdesc);
	}
...
static enum ch_dev_hsize_t slave_buswidth_to_hsize(enum dma_slave_buswidth buswidth)
{
	enum ch_dev_hsize_t hsize = DEV_HSIZE_INVALID;
		...
		enum dma_transfer_direction dir)
{
	...
	//"generate hwdesc(%px): ctl=%x, cfg0=%x, cfg1=%x, dev_tout=%x, dev_sel=%x, src_addr=%x,
	//dst_addr=%x\n",
	//hwdesc, hwdesc->ctl, hwdesc->cfg0, hwdesc->cfg1, hwdesc->dev_tout, hwdesc->dev_sel,
			   ...
			   unsigned long flags, void *context)
{
	...
	size_t xfer_size, avail = 0;
	dma_addr_t addr, src = 0, dst = 0;
	int i = 0;

	dev_vdbg(vchan2dev(vchan),
		 ": prep_slave_sg: sgl: %p sglen: %ud flags: %#lx", sgl, sglen,
		 flags);
	...
	for_each_sg(sgl, sg, sglen, i) {
		...
		do {
			...
			dev_vdbg(
				vchan2dev(vchan),
				"dma_async_tx_descriptor: src=%llx, dst=%llx, xfer_size=%zu\n",
				src, dst, xfer_size);

			hwdesc = generate_hwdesc(vchan, src, dst, xfer_size,
						 sconfig, dir);
			if (hwdesc)
				list_add_tail(&hwdesc->list, &desc->xfer_list);
			else {
				list_for_each_entry_safe(hwdesc, tmp,
							 &desc->xfer_list, list)
					kfree(hwdesc);

				kfree(desc);
				dev_err(vchan2dev(vchan),
					"dma_async_tx_descriptor: generate hwdesc failure\n");
				return NULL;
			}
	...
	dev_vdbg(vchan2dev(vchan),
		 "dma_async_tx_descriptor: prepare desc(%p)\n", desc);
					      ...
					      struct dma_tx_state *txstate)
{
	...
	if (ret != DMA_COMPLETE) {
		...
		if (vdesc) {
			residue = k230_peridma_desc_residue(
				vchan, vd_to_k230_peridma_desc(vdesc));
		}
	}
	spin_unlock_irqrestore(&vchan->vchan.lock, flags);
...
static struct k230_peridma_pchan *
find_phy_chan(struct k230_peridma_dev *pdev, struct k230_peridma_vchan *vchan)
{
...
static void peridma_vchan_start_first_queued(struct k230_peridma_vchan *vchan)
{
	...
	if (vchan->pchan) {
		pchan = vchan->pchan;
		dev_vdbg(vchan2dev(vchan),
			 "%s already allocated phy channel\n",
			 peridma_vchan_name(vchan));
	} else {
	...
	if (!list_empty(&desc->xfer_list)) {
		...
		// hwdesc);
		//dev_vdbg(vchan2dev(vchan),
		//" ctl=%x, cfg0=%x, cfg1=%x, dev_tout=%x, dev_sel=%x, src_addr=%x, dst_addr=%x\n",
		//hwdesc->ctl, hwdesc->cfg0, hwdesc->cfg1, hwdesc->dev_tout, hwdesc->dev_sel,
		//hwdesc->src_addr, hwdesc->dst_addr);
		vchan->desc = desc;
		desc->hwdesc = hwdesc;
...
static void peridma_vchan_xfer_complete(struct k230_peridma_vchan *vchan)
{
	...
	if (unlikely(!peridma_pchan_is_idle(vchan->pchan))) {
		dev_err(vchan2dev(vchan),
			"BUG: %s caught chx_tr_done, but channel not idle!\n",
			peridma_vchan_name(vchan));
		peridma_pchan_stop(vchan->pchan);
	}
...
static void k230_peridma_issue_pending(struct dma_chan *dchan)
{
	...
	if (vchan_issue_pending(&vchan->vchan)) {
		dev_vdbg(vchan2dev(vchan), "%s: 1\n", __func__);
		peridma_vchan_start_first_queued(vchan);
	}
	dev_vdbg(vchan2dev(vchan), "%s: 2\n", __func__);
...
static irqreturn_t k230_peridma_interrupt(int irq, void *dev_id)
{
	...
	for (i = 0; i < CH_NUM; i++) {
		...
		if (desc->cyclic) {
			hwdesc = desc->cyclic;
			//dev_vdbg(vchan2dev(vchan),
			//"hwdesc(%px): last=%d ctl=%x, cfg0=%x,\n cfg1=%x,
			//dev_tout=%x, dev_sel=%x, src_addr=%x, dst_addr=%x\n",
			//hwdesc, hwdesc->last, hwdesc->ctl, hwdesc->cfg0, hwdesc->cfg1,
			//hwdesc->dev_tout, hwdesc->dev_sel,
			//hwdesc->src_addr, hwdesc->dst_addr);
			...
			if (desc->cyclic->list.next == &(desc->xfer_list))
				desc->cyclic = list_entry(
					desc->xfer_list.next,
					typeof(*(desc->cyclic)), list);
			else
				desc->cyclic =
					list_next_entry(desc->hwdesc, list);
			...
			if (!list_empty(&desc->xfer_list)) {
				...
				//dev_vdbg(vchan2dev(vchan),
				//"hwdesc(%px): ctl=%x, cfg0=%x, cfg1=%x, dev_tout=%x, dev_sel=%x, \
				//src_addr=%x, dst_addr=%x\n",
				//hwdesc, hwdesc->ctl, hwdesc->cfg0, hwdesc->cfg1, hwdesc->dev_tout,
					   ...
					   struct of_dma *ofdma)
{
	...
	dev_vdbg(
		priv->dev,
		"%s: priority=%x, dev_tout=%x, dat_endian=%x, dev_sel=%x\n",
		__func__, priority, dev_tout, dat_endian, dev_sel);
	...
	dev_vdbg(priv->dev, "%s: get vchan %s\n",
		__func__, peridma_vchan_name(vchan));
	return chan;
}
...
static int k230_peridma_probe(struct platform_device *pdev)
{
	...
	priv->pchan = devm_kcalloc(priv->dev, priv->nr_channels,
				   sizeof(struct k230_peridma_pchan),
				   GFP_KERNEL);
	if (!priv->pchan)
		return -ENOMEM;
	...
	priv->vchan = devm_kcalloc(priv->dev, priv->nr_requests,
				   sizeof(struct k230_peridma_vchan),
				   GFP_KERNEL);
	if (!priv->vchan)
		return -ENOMEM;
	...
	priv->slave.directions = BIT(DMA_DEV_TO_MEM) | BIT(DMA_MEM_TO_DEV);
	...
	priv->slave.device_free_chan_resources =
		k230_peridma_vchan_free_chan_resources;
	...
	ret = devm_request_irq(&pdev->dev, priv->irq, k230_peridma_interrupt, 0,
	...
	dev_vdbg(
		&pdev->dev,
		"Successfully probed K230 peridma controller, register mapped at %p(PHY %llu)\n",
		priv->base, res->start);
