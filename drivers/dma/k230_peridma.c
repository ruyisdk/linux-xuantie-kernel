	...
	enum dma_transfer_direction dir, unsigned long flags, void *context)
{
	...
	int i = 0;
        ...
        dev_vdbg(vchan2dev(vchan), ": prep_slave_sg: sgl: %px sglen: %ud flags: %#lx",
                sgl, sglen, flags);
...
//	desc->next = false;
	...
	for_each_sg(sgl, sg, sglen, i) {
		...
		do {
			...
			dev_vdbg(vchan2dev(vchan), "dma_async_tx_descriptor: src=%llx, dst=%llx, xfer_size=%zu \n", src, dst, xfer_size);
...
//        priv->slave.device_alloc_chan_resources	= k230_peridma_vchan_alloc_chan_resources;
	...
	dev_vdbg(&pdev->dev, "Successfully probed K230 peridma controller, register mapped at %pllx(PHY %llu) \n", priv->base ,res->start);
