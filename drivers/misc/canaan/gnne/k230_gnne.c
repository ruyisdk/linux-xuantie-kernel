...
typedef struct gnne_mem_info {
    void *user_virt_addr;
    void *kernel_virt_addr;
    unsigned long gnne_phys;
...
static int gnne_open(struct inode *inode, struct file *filp)
{
    ...
    if (!sysctl_reg) {
        printk(KERN_ERR "can't remap gnne sysctl 0x%08X\n", 0x91103028);
        return -1;
    }
...
static int gnne_probe(struct platform_device *pdev)
{
    ...
    if (!res) {
        dev_dbg(&pdev->dev, "gnne_probe: get resource failed \n");
        err = -ENODEV;
        goto error;
    }
    ...
    if (!plat) {
        dev_dbg(&pdev->dev, "gnne_probe: kzalloc failed \n");
        err = -ENOMEM;
        goto error;
    }
    ...
    if(plat->irq < 0) {
        dev_dbg(&pdev->dev, "Gnne get irq err\n");
        err = -ENODEV;
        goto cleanup_kmem;
    }

    if((err = request_irq(plat->irq, gnne_irq, 0, "t0", NULL))) {
        dev_dbg(&pdev->dev, "Gnne request irq err %d\n", err);
        goto cleanup_kmem;
    }
    ...
    if (err) {
        dev_dbg(&pdev->dev, "k230-gnne: can't get major %d\n", plat->major);
    ...
    if (err) {
        dev_dbg(&pdev->dev, "Error %d adding gnne device number %d \n", err, plat->minor);
        goto cleanup_class;
    }

    plat->device = device_create(plat->class, NULL, devno, NULL, "k230-gnne");
    if (IS_ERR(plat->device)) {
        dev_dbg(&pdev->dev, "device not created\n");
