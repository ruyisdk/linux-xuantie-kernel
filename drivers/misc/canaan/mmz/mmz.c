/*
 * kernel module helper for testing MMZ
 *
...
static long mmz_ioctl(struct file *filp, unsigned int cmd,
                             unsigned long arg)
{
    mmz_info_t t_mem_info;
    switch (cmd) {
        case MMZ_ALLOC_MEM :
            if(copy_from_user(&t_mem_info, (void *)arg, sizeof(mmz_info_t)))
                return -EFAULT;
            t_mem_info.kernel_virt_addr = dma_alloc_coherent(mmz_dev, t_mem_info.length, (dma_addr_t*)(&(t_mem_info.mmz_phys)), GFP_KERNEL | GFP_DMA);
            if(copy_to_user((void *)arg, &t_mem_info, sizeof(mmz_info_t)))
                return -EFAULT;
            break;
        case MMZ_FREE_MEM :
            if(copy_from_user(&t_mem_info, (void *)arg, sizeof(mmz_info_t)))
                return -EFAULT;
            dma_free_coherent(mmz_dev, t_mem_info.length, t_mem_info.kernel_virt_addr, t_mem_info.mmz_phys);
            break;
        default:
            printk(KERN_ERR "mmz_ioctl: Unknown ioctl: 0x%.8X\n", cmd);
            return -EINVAL;
    }
    return 0;
}

static long mmz_unlocked_ioctl(struct file *file, unsigned int cmd, unsigned long arg)
{
    long ret = 0;
...
static int mmz_map(struct file *file, struct vm_area_struct *vma)
{
	...
	if(offset < 0x80000000)  // memory set no SO, cached
	{
...
MODULE_DESCRIPTION("kernel module to help the test of MMZ");
MODULE_ALIAS("MMZ test");
