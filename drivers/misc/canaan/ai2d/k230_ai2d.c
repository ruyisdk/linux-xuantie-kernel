...
static int ai2d_probe(struct platform_device *pdev)
{
    ...
    if (!res) {
        dev_dbg(&pdev->dev, "ai2d_probe: get resource failed \n");
        err = -ENODEV;
        goto error;
    }
    ...
    if (!plat) {
        dev_dbg(&pdev->dev, "ai2d_probe: kzalloc failed \n");
        err = -ENOMEM;
        goto error;
    }
    ...
    dev_dbg(&pdev->dev, "Gnne irq number is %d\n", plat->irq);
    if(plat->irq < 0) {
    ...
    if (err) {
        dev_dbg(&pdev->dev, "k230-ai2d: can't get major %d\n", plat->major);
    ...
    if (err) {
        dev_dbg(&pdev->dev, "Error %d adding ai2d device number %d \n", err, plat->minor);
        goto cleanup_class;
    }
