...
static void watchdog_start(struct mvx_session *session, unsigned int timeout_ms)
{
	int ret;
...
static int wait_pending(struct mvx_session *session)
{
	...
	while (is_fw_loaded(session) != false && session->fw.msg_pending > 0 &&
	       session->error == 0) {
...
{
	struct mvx_fw_msg msg = { .code = MVX_FW_CODE_STATE_CHANGE,
				  .state = state };
	int ret = 0;
...
{
	struct mvx_fw_msg msg = { .code = MVX_FW_CODE_JOB,
...
{
	struct mvx_fw_msg msg = { .code = MVX_FW_CODE_DEBUG,
				  .arg = debug_level };
...
{
	struct mvx_fw_msg msg = { .code = MVX_FW_CODE_SET_OPTION,
				  .set_option = *option };
...
{
	struct mvx_fw_msg msg = { .code = MVX_FW_CODE_EOS,
				  .eos_is_frame = is_encoder(session) ? true :
									false };
	int ret;
...
static int fw_set_epr_qp(struct mvx_session *session, int code,
			 struct mvx_buffer_param_qp qp)
{
	...
	if (ret != 0) {
		MVX_SESSION_WARN(session,
				 "Failed to set EPR QP. code=%d, ret=%d.", code,
				 ret);
		return ret;
	}
...
static int fw_set_qp(struct mvx_session *session, int code, int qp)
{
	...
	ret = fw_set_option(session, &option);
	if (ret != 0) {
		MVX_SESSION_WARN(session, "Failed to set QP. code=%d, ret=%d.",
				 code, ret);
		return ret;
	}
...
static int fw_set_osd_config(struct mvx_session *session, int code,
...
static int fw_set_roi_regions(struct mvx_session *session, int code,
			      struct mvx_roi_config *roi)
{
	...
	ret = fw_set_option(session, &option);
	if (ret != 0) {
		MVX_SESSION_WARN(session, "Failed to set ROI. code=%d, ret=%d.",
				 code, ret);
		return ret;
	}
...
static int fw_set_chr_cfg(struct mvx_session *session, int code,
...
static int fw_set_enc_stats(struct mvx_session *session, int code,
			    struct mvx_enc_stats *stats)
{
	...
	if (ret != 0) {
		MVX_SESSION_WARN(
			session,
			"Failed to set enc stats param. code=%d, ret=%d.", code,
			ret);
		return ret;
	}
...
void generate_quant_tbl(int quality, const uint8_t qtbl_ref[MVX_FW_QUANT_LEN],
...
static int
generate_standards_yuv2rgb_coef(enum mvx_yuv_to_rgb_mode mode,
				struct mvx_color_conv_coef *color_conv_coef)
{
	...
	MVX_LOG_PRINT(
		&mvx_log_if, MVX_LOG_DEBUG,
		"%s.mode indx=%d 3x3=[%d %d %d, %d %d %d,%d %d %d],offset=[%d %d %d]", __func__,
		mode, color_conv_coef->coef[0][0], color_conv_coef->coef[0][1],
		color_conv_coef->coef[0][2], color_conv_coef->coef[1][0],
		color_conv_coef->coef[1][1], color_conv_coef->coef[1][2],
		color_conv_coef->coef[2][0], color_conv_coef->coef[2][1],
		color_conv_coef->coef[2][2], color_conv_coef->offset[0],
		color_conv_coef->offset[1], color_conv_coef->offset[2]);

	return 0;
}

static int generate_standards_rgb2yuv_coef(
	enum mvx_rgb_to_yuv_mode mode,
	struct mvx_rgb2yuv_color_conv_coef *color_conv_coef)
{
...
static int fw_encoder_setup(struct mvx_session *session)
{
	...
	MVX_SESSION_DEBUG(session, "%s>encoder settings:\n", __func__);
	MVX_SESSION_DEBUG(session, "codec %d\n", codec);
	MVX_SESSION_DEBUG(session, "frame_rate 0x%lx\n", session->frame_rate);
	MVX_SESSION_DEBUG(session, "target_bitrate %d\n", session->target_bitrate);
	MVX_SESSION_DEBUG(session, "maximum_bitrate %d\n", session->maximum_bitrate);
	MVX_SESSION_DEBUG(session, "rc_enabled %d\n", session->rc_enabled);
	MVX_SESSION_DEBUG(session, "rc_type %d\n", session->rc_type);
	MVX_SESSION_DEBUG(session, "profile %d\n", session->profile[codec]);
	MVX_SESSION_DEBUG(session, "level %d\n", session->level[codec]);
	MVX_SESSION_DEBUG(session, "nalu_format %d\n", session->nalu_format);
	MVX_SESSION_DEBUG(session, "stream_escaping %d\n", session->stream_escaping);
	MVX_SESSION_DEBUG(session, "ignore_stream_headers %d\n", session->ignore_stream_headers);
	MVX_SESSION_DEBUG(session, "frame_reordering %d\n", session->frame_reordering);
	MVX_SESSION_DEBUG(session, "intbuf_size %ld\n", session->intbuf_size);
	MVX_SESSION_DEBUG(session, "p_frames %d\n", session->p_frames);
	MVX_SESSION_DEBUG(session, "b_frames %d\n", session->b_frames);
	MVX_SESSION_DEBUG(session, "gop_type %d\n", session->gop_type);
	MVX_SESSION_DEBUG(session, "cyclic_intra_refresh_mb %d\n",
				session->cyclic_intra_refresh_mb);
	MVX_SESSION_DEBUG(session, "constr_ipred %d\n", session->constr_ipred);
	MVX_SESSION_DEBUG(session, "entropy_sync %d\n", session->entropy_sync);
	MVX_SESSION_DEBUG(session, "temporal_mvp %d\n", session->temporal_mvp);
	MVX_SESSION_DEBUG(session, "tile_rows %d\n", session->tile_rows);
	MVX_SESSION_DEBUG(session, "tile_cols %d\n", session->tile_cols);
	MVX_SESSION_DEBUG(session, "min_luma_cb_size %d\n", session->min_luma_cb_size);
	MVX_SESSION_DEBUG(session, "mb_mask %d\n", session->mb_mask);
	MVX_SESSION_DEBUG(session, "entropy_mode %d\n", session->entropy_mode);
	MVX_SESSION_DEBUG(session, "multi_slice_mode %d\n", session->multi_slice_mode);
	MVX_SESSION_DEBUG(session, "multi_slice_max_mb %d\n", session->multi_slice_max_mb);
	MVX_SESSION_DEBUG(session, "vp9_prob_update %d\n", session->vp9_prob_update);
	MVX_SESSION_DEBUG(session, "mv_h_search_range %d\n", session->mv_h_search_range);
	MVX_SESSION_DEBUG(session, "mv_v_search_range %d\n", session->mv_v_search_range);
	MVX_SESSION_DEBUG(session, "bitdepth_chroma %d\n", session->bitdepth_chroma);
	MVX_SESSION_DEBUG(session, "bitdepth_luma %d\n", session->bitdepth_luma);
	MVX_SESSION_DEBUG(session, "force_chroma_format %d\n", session->force_chroma_format);
	MVX_SESSION_DEBUG(session, "rgb_to_yuv %d\n", session->rgb_to_yuv);
	MVX_SESSION_DEBUG(session, "band_limit %d\n", session->band_limit);
	MVX_SESSION_DEBUG(session, "cabac_init_idc %d\n", session->cabac_init_idc);
	MVX_SESSION_DEBUG(session, "qp %d, %d, %d, %d, %d\n", session->qp[codec].i_frame,
				session->qp[codec].p_frame, session->qp[codec].b_frame,
				session->qp[codec].min, session->qp[codec].max);
	MVX_SESSION_DEBUG(session, "resync_interval %d\n", session->resync_interval);
	MVX_SESSION_DEBUG(session, "jpeg_quality %d\n", session->jpeg_quality);
	MVX_SESSION_DEBUG(session, "color_desc %d, %d, %d, %d, %d, %d, %d, %d, %d, %d, %d\n",
				session->color_desc.flags, session->color_desc.range,
				session->color_desc.primaries, session->color_desc.transfer,
				session->color_desc.matrix,
				session->color_desc.video_format,
				session->color_desc.aspect_ratio_idc,
				session->color_desc.sar_width,
				session->color_desc.sar_height,
				session->color_desc.num_units_in_tick,
				session->color_desc.time_scale);
	MVX_SESSION_DEBUG(session, "crop_left %d\n", session->crop_left);
	MVX_SESSION_DEBUG(session, "crop_right %d\n", session->crop_right);
	MVX_SESSION_DEBUG(session, "crop_top %d\n", session->crop_top);
	MVX_SESSION_DEBUG(session, "crop_bottom %d\n", session->crop_bottom);
	MVX_SESSION_DEBUG(session, "sei_userdata %d\n", session->sei_userdata.flags);
	MVX_SESSION_DEBUG(session, "nHRDBufsize %d\n", session->nHRDBufsize);
	MVX_SESSION_DEBUG(session, "dsl_frame %d, %d\n",
				session->dsl_frame.width, session->dsl_frame.height);
	MVX_SESSION_DEBUG(session, "dsl_ratio %d, %d\n",
				session->dsl_ratio.hor, session->dsl_ratio.ver);
	MVX_SESSION_DEBUG(session, "mvx_ltr %d, %d\n",
				session->mvx_ltr.mode, session->mvx_ltr.period);
	MVX_SESSION_DEBUG(session, "dsl_pos_mode %d\n", session->dsl_pos_mode);
	MVX_SESSION_DEBUG(session, "mini_frame_height %d\n", session->mini_frame_height);
	MVX_SESSION_DEBUG(session, "init_qpi %d\n", session->init_qpi);
	MVX_SESSION_DEBUG(session, "init_qpp %d\n", session->init_qpp);
	MVX_SESSION_DEBUG(session, "sao_luma %d\n", session->sao_luma);
	MVX_SESSION_DEBUG(session, "sao_chroma %d\n", session->sao_chroma);
	MVX_SESSION_DEBUG(session, "qp_delta_i_p %d\n", session->qp_delta_i_p);
	MVX_SESSION_DEBUG(session, "ref_rb_en %d\n", session->ref_rb_en);
	MVX_SESSION_DEBUG(session, "qpmap_qp_clip_top %d\n", session->qpmap_qp_clip_top);
	MVX_SESSION_DEBUG(session, "qpmap_qp_clip_bot %d\n", session->qpmap_qp_clip_bot);
	MVX_SESSION_DEBUG(session, "rc_qp_clip_top %d\n", session->rc_qp_clip_top);
	MVX_SESSION_DEBUG(session, "rc_qp_clip_bot %d\n", session->rc_qp_clip_bot);
	MVX_SESSION_DEBUG(session, "max_qp_i %d\n", session->max_qp_i);
	MVX_SESSION_DEBUG(session, "min_qp_i %d\n", session->min_qp_i);
	MVX_SESSION_DEBUG(session, "profiling %d\n", session->profiling);
	MVX_SESSION_DEBUG(session, "visible_width %d\n", session->visible_width);
	MVX_SESSION_DEBUG(session, "visible_height %d\n", session->visible_height);
	MVX_SESSION_DEBUG(session, "rc_bit_i_mode %d\n", session->rc_bit_i_mode);
	MVX_SESSION_DEBUG(session, "rc_bit_i_ratio %d\n", session->rc_bit_i_ratio);
	MVX_SESSION_DEBUG(session, "inter_med_buf_size %d\n", session->inter_med_buf_size);
	MVX_SESSION_DEBUG(session, "svct3_level1_period %d\n", session->svct3_level1_period);
	MVX_SESSION_DEBUG(session, "reset_gop_pframes %d\n", session->reset_gop_pframes);
	MVX_SESSION_DEBUG(session, "reset_ltr_period %d\n", session->reset_ltr_period);
	MVX_SESSION_DEBUG(session, "fixedqp %d\n", session->fixedqp);
	MVX_SESSION_DEBUG(session, "gdr_number %d\n", session->gdr_number);
	MVX_SESSION_DEBUG(session, "gdr_period %d\n", session->gdr_period);
	MVX_SESSION_DEBUG(session, "multi_sps_pps %d\n", session->multi_sps_pps);
	MVX_SESSION_DEBUG(session, "enable_visual %d\n", session->enable_visual);
	MVX_SESSION_DEBUG(session, "scd_enable %d\n", session->scd_enable);
	MVX_SESSION_DEBUG(session, "scd_percent %d\n", session->scd_percent);
	MVX_SESSION_DEBUG(session, "scd_threshold %d\n", session->scd_threshold);
	MVX_SESSION_DEBUG(session, "aq_ssim_en %d\n", session->aq_ssim_en);
	MVX_SESSION_DEBUG(session, "aq_neg_ratio %d\n", session->aq_neg_ratio);
	MVX_SESSION_DEBUG(session, "aq_pos_ratio %d\n", session->aq_pos_ratio);
	MVX_SESSION_DEBUG(session, "aq_qpdelta_lmt %d\n", session->aq_qpdelta_lmt);
	MVX_SESSION_DEBUG(session, "aq_init_frm_avg_svar %d\n", session->aq_init_frm_avg_svar);
	MVX_SESSION_DEBUG(session, "adaptive_intra_block %d\n", session->adaptive_intra_block);
	MVX_SESSION_DEBUG(session, "seamless_mode %d, w %d, h %d\n",
				session->seamless_target.seamless_mode,
				session->seamless_target.target_width,
				session->seamless_target.target_height);
	MVX_SESSION_DEBUG(session, "color_conv_mode %d\n", session->color_conv_mode);
	MVX_SESSION_DEBUG(session, "use_cust_rgb_to_yuv_mode %d\n",
				session->use_cust_rgb_to_yuv_mode);
	MVX_SESSION_DEBUG(session, "use_cust_color_conv_coef %d\n",
				session->use_cust_color_conv_coef);
	MVX_SESSION_DEBUG(session, "forced_uv_value %d\n", session->forced_uv_value);
	MVX_SESSION_DEBUG(session, "dsl_interp_mode %d\n", session->dsl_interp_mode);
	MVX_SESSION_DEBUG(session, "disabled_features %d\n", session->disabled_features);
	MVX_SESSION_DEBUG(session, "change_pos %d\n", session->change_pos);
	MVX_SESSION_DEBUG(session, "enc_src_crop.crop_en %d\n", session->enc_src_crop.crop_en);
	...
	if (codec != MVX_FORMAT_JPEG) {
		if (session->rc_type) {
			...
			option.rate_control.rate_control_mode =
				session->rc_type;
			if (session->rc_type ==
	...
	if ((codec == MVX_FORMAT_HEVC || codec == MVX_FORMAT_VP9) &&
	    (session->tile_rows != 0 || session->tile_cols != 0)) {
	...
	if (codec == MVX_FORMAT_H264 || codec == MVX_FORMAT_HEVC) {
		...
		if (session->color_desc.range != 0 ||
		    session->color_desc.matrix != 0 ||
		    session->color_desc.primaries != 0 ||
		    session->color_desc.transfer != 0 ||
		...
		if (session->mvx_ltr.mode != 0 ||
		    session->mvx_ltr.period != 0) {
	...
	if (session->use_cust_rgb_to_yuv_mode ==
	...
	if (codec == MVX_FORMAT_JPEG) {
		...
		if (session->jpeg_quality != 0 ||
			...
			if (quality_luma) {
				generate_quant_tbl(quality_luma, qtbl_luma_ref,
						   qtbl_luma);
	...
	if (session->enc_src_crop.width != 0 &&
		...
		memcpy(&option.enc_src_crop, &session->enc_src_crop,
...
static int fw_decoder_setup(struct mvx_session *session)
{
	...
	if (mvx_is_rgb24(session->port[MVX_DIR_OUTPUT].format)) {
		...
		if (session->use_cust_color_conv_coef) {
			...
			ret = generate_standards_yuv2rgb_coef(
				session->color_conv_mode,
	...
	if (session->dec_dst_crop.crop_en != 0 &&
		...
		memcpy(&option.dec_dst_crop, &session->dec_dst_crop,
		       sizeof(struct mvx_crop_cfg));
		ret = fw_set_option(session, &option);
		if (ret != 0) {
			MVX_SESSION_WARN(session,
 ...
 * Return 0 on success, else error code.
 */
static int map_buffer(struct mvx_session *session, enum mvx_direction dir,
			     ...
			     struct mvx_roi_config *roi_cfg)
{
	...
	if (roi_cfg->roi_present)
		ret = fw_set_roi_regions(session, MVX_FW_SET_ROI_REGIONS, roi_cfg);
 ...
 * Return: 0 on success, else error code.
 */
static int queue_buffer(struct mvx_session *session, enum mvx_direction dir,
			struct mvx_buffer *buf)
{
	...
	if (dir == MVX_DIR_OUTPUT &&
	    port->buffer_allocated < port->buffer_min) {
	...
	if (dir == MVX_DIR_OUTPUT &&
		...
		width = seamless->target_width < port->width ?
				port->width :
				seamless->target_width;
		height = seamless->target_height < port->height ?
				 port->height :
				 seamless->target_height;
		for (i = 0; i < MVX_BUFFER_NPLANES; i++) {
			stride[i] = seamless->target_stride[i] <
						    port->stride[i] ?
					    port->stride[i] :
	...
	if (mvx_is_afbc(port->format) != false) {
		port->afbc_width = DIV_ROUND_UP(
			width, 16 << (!!(buf->flags &
					 MVX_BUFFER_AFBC_32X8_SUPERBLOCK)));
		ret = mvx_buffer_afbc_set(buf, port->format, width, height,
					  port->afbc_width, port->size[0],
					  port->interlaced);
		if (ret != 0)
			return ret;
	} else if (mvx_is_frame(port->format) != false) {
		ret = mvx_buffer_frame_set(buf, port->format, width, height,
					   stride, port->size,
					   port->interlaced);
		if (ret != 0)
			return ret;
	...
	MVX_SESSION_INFO(
		...
		buf->dir, buf->planes[0].filled, buf->planes[1].filled,
		buf->planes[2].filled, buf->flags,
				 ...
				 enum mvx_direction dir)
{
	...
	list_for_each_entry_safe(buf, tmp, &session->port[dir].buffer_queue,
		...
		if ((buf->flags & MVX_BUFFER_FRAME_FLAG_ROI) ==
			...
			roi_config =
				session->port[dir]
					.roi_config_queue[roi_config_index];
		...
		if ((buf->flags & MVX_BUFFER_FRAME_FLAG_CHR) ==
			    MVX_BUFFER_FRAME_FLAG_CHR &&
		    chr_cfg_index < chr_cfg_num) {
			ret = queue_chr_cfg(
				session,
				&session->port[dir]
					 .chr_cfg_queue[chr_cfg_index]);
			chr_cfg_index++;
		}
		if (enc_stats_index < enc_stats_num &&
		    session->port[dir]
				    .enc_stats_queue[enc_stats_index]
				    .pic_index == pending_buf_idx) {
			ret = queue_enc_stats(
				session,
				&session->port[dir]
					 .enc_stats_queue[enc_stats_index]);
		...
		if (osd_cfg_index < osd_cfg_num &&
		    session->port[dir].osd_cfg_queue[osd_cfg_index].pic_index ==
			    osd_buffer_idx) {
			ret = queue_osd_config(
				session,
				&session->port[dir]
					 .osd_cfg_queue[osd_cfg_index]);
			osd_cfg_index++;
		}
...
static void fw_bin_ready(struct mvx_fw_bin *bin, void *arg, bool same_thread)
{
	...
	session->isession.ncores =
		session->client_ops->get_ncores(session->client_ops);
	...
	ret = mvx_fw_factory(&session->fw, bin, &session->mmu, session,
			     session->client_ops, session->csession,
			     session->isession.ncores, session->dentry);
	if (ret != 0) {
		send_event_error(session, ret);
...
static int calc_afbc_size(struct mvx_session *session, enum mvx_format format,
			  unsigned int width, unsigned int height,
			  bool tiled_headers, bool tiled_body, bool superblock,
			  bool interlaced)
{
	...
	case MVX_FORMAT_YUV422_AFBC_10:
		...
		break;
	default:
		MVX_SESSION_WARN(session, "Unsupported AFBC format. format=%u.",
				 format);
		return -EINVAL;
	}
...
static int try_format(struct mvx_session *session, enum mvx_direction dir,
		      enum mvx_format format, unsigned int *width,
		      unsigned int *height, uint8_t *nplanes,
		      ...
		      bool *interlaced)
{
	...
	*height = min_t(unsigned int, *height, 32768);
	...
	if (dir == MVX_DIR_OUTPUT) {
		*width = session->port[MVX_DIR_INPUT].width >>
			 session->port[MVX_DIR_OUTPUT].scaling_shift;
		*height = session->port[MVX_DIR_INPUT].height >>
			  session->port[MVX_DIR_OUTPUT].scaling_shift;
	}
	if (session->dsl_frame.width != 0 && session->dsl_frame.height != 0) {
		*width = session->dsl_frame.width;
		*height = session->dsl_frame.height;
	} else if (dir == MVX_DIR_OUTPUT && (session->dsl_ratio.hor != 1 ||
					     session->dsl_ratio.ver != 1)) {
		*width = session->port[MVX_DIR_INPUT].width /
			 session->dsl_ratio.hor;
		*height = session->port[MVX_DIR_INPUT].height /
			  session->dsl_ratio.ver;
		*width &= ~(1);
		*height &= ~(1);
	}
	if (dir == MVX_DIR_OUTPUT && mvx_is_frame(format)) {
		if (session->dec_dst_crop.crop_en != 0 &&
		    session->dec_dst_crop.width > 0 &&
		    session->dec_dst_crop.height > 0 &&
		    session->dec_dst_crop.width + session->dec_dst_crop.x <=
			    session->port[MVX_DIR_INPUT].width &&
		    session->dec_dst_crop.height + session->dec_dst_crop.y <=
	...
	if (mvx_is_afbc(format) != false) {
		...
		if (dir == MVX_DIR_INPUT) {
			/* it is basically a worst-case calcualtion based on
			 * a size rounded up to tile size
			 */
			int s1 = calc_afbc_size(
				session, format, *width, *height, true, true,
				false, //*height, false, false, false,
				*interlaced);
			int s2 = calc_afbc_size(
				session, format, *width, *height, true, true,
				true, //*height, false, false, false,
				*interlaced);
		...
		size[0] = max_t(unsigned int, size[0], afbc_alloc_bytes);
		size[0] = roundup(size[0], PAGE_SIZE);

		*nplanes = 1;
	} else if (mvx_is_frame(format) != false) {
		uint32_t tmp_height = session->mini_frame_height >= 64 ?
					      session->mini_frame_height :
					      *height;
		...
		size[0] = min_t(
			unsigned int, size[0],
			(8192) * (8192));
		/* for large jpeg,bit stream buffer no need to set too large,
		 * just clip to 8k*8k
		 */
		size[0] = roundup(size[0], PAGE_SIZE);
...
int mvx_session_construct(struct mvx_session *session, struct device *dev,
			  struct mvx_client_ops *client_ops,
			  struct mvx_fw_cache *cache, struct mutex *mutex,
			  ...
			  struct dentry *dentry)
{
	...
	memset(session->port[MVX_DIR_INPUT].stride, 0,
	       sizeof(session->port[MVX_DIR_INPUT].stride));
	memset(session->port[MVX_DIR_INPUT].size, 0,
	       sizeof(session->port[MVX_DIR_INPUT].size));
	memset(session->port[MVX_DIR_OUTPUT].stride, 0,
	       sizeof(session->port[MVX_DIR_OUTPUT].stride));
	memset(session->port[MVX_DIR_OUTPUT].size, 0,
	       sizeof(session->port[MVX_DIR_OUTPUT].size));
...
int mvx_session_try_format(struct mvx_session *session, enum mvx_direction dir,
			   enum mvx_format format, unsigned int *width,
			   unsigned int *height, uint8_t *nplanes,
			   unsigned int *stride, unsigned int *size,
			   bool *interlaced)
...
int mvx_session_set_format(struct mvx_session *session, enum mvx_direction dir,
			   enum mvx_format format, unsigned int *width,
			   unsigned int *height, uint8_t *nplanes,
			   unsigned int *stride, unsigned int *size,
			   bool *interlaced)
{
	...
	ret = try_format(session, dir, format, width, height, nplanes, stride,
			 size, interlaced);
	if (ret != 0)
		return ret;
...
int mvx_session_qbuf(struct mvx_session *session, enum mvx_direction dir,
		     struct mvx_buffer *buf)
{
	int ret;
...
int mvx_session_streamon(struct mvx_session *session, enum mvx_direction dir)
{
	...
	for (i = MVX_DIR_INPUT; i < MVX_DIR_MAX; i++) {
		...
		session->client_ops->get_formats(session->client_ops, i,
						 &formats);
	...
	ret = mvx_fw_cache_get(session->cache, session->port[bdir].format, bdir,
			       &session->fw_event, &hw_ver,
			      ...
			      struct mvx_fw_msg *msg)
{
	...
	case MVX_FW_CODE_ALLOC_PARAM: {
		...
		MVX_SESSION_INFO(
			...
			msg->alloc_param.width, msg->alloc_param.height,
			output->nplanes, output->size[0], output->size[1],
			output->size[2], output->stride[0], output->stride[1],
	...
	case MVX_FW_CODE_BUFFER: {
		...
		if (buf->dir == MVX_DIR_OUTPUT &&
			...
			mvx_buffer_frame_dim(output->format, buf->width,
					     ...
					     session->setting_stride);
			for (i = 0; i < buf->nplanes; i++) {
				(void)mvx_buffer_filled_set(buf, i, filled[i],
							    0);
			}
			...
			MVX_SESSION_INFO(
				...
				buf->dir, filled[0], filled[1], filled[2],
	...
	case MVX_FW_CODE_DISPLAY_SIZE: {
		if (session->port[MVX_DIR_INPUT].format == MVX_FORMAT_AV1) {
			...
			mvx_buffer_frame_dim(output->format,
					     ...
					     session->setting_stride);
			for (i = 0; i < MVX_BUFFER_NPLANES; i++) {
				session->setting_stride[i] = max_t(
					unsigned int,
					session->setting_stride[i], stride[i]);
			}
			MVX_SESSION_INFO(
				session,
...
int mvx_session_set_frame_rate(struct mvx_session *session, int64_t frame_rate)
{
	int ret;
...
int mvx_session_set_profile(struct mvx_session *session, enum mvx_format format,
...
int mvx_session_set_level(struct mvx_session *session, enum mvx_format format,
...
int mvx_session_set_p_frames(struct mvx_session *session, int val)
{
	if (session->error != 0)
		return session->error;
...
int mvx_session_set_i_frame_qp(struct mvx_session *session, enum mvx_format fmt,
			       int qp)
{
	int ret;
...
int mvx_session_set_p_frame_qp(struct mvx_session *session, enum mvx_format fmt,
			       int qp)
{
	int ret;
...
int mvx_session_set_b_frame_qp(struct mvx_session *session, enum mvx_format fmt,
			       int qp)
{
	int ret;
...
int mvx_session_set_min_qp(struct mvx_session *session, enum mvx_format fmt,
			   int qp)
{
	int ret;
...
int mvx_session_set_max_qp(struct mvx_session *session, enum mvx_format fmt,
			   int qp)
{
	int ret;
	...
	if (is_fw_loaded(session) == false ||
		...
		if (roi_config_num < MVX_ROI_QP_NUMS) {
			...
			session->port[MVX_DIR_INPUT]
				.roi_config_queue[roi_config_num] = *roi;
...
int mvx_session_set_mini_frame_height(struct mvx_session *session, int *height)
{
	if (session->error != 0)
		return session->error;
	...
	if (is_fw_loaded(session) == false ||
		...
		if (enc_stats_num < MVX_ROI_QP_NUMS) {
			MVX_SESSION_INFO(
				...
				enc_stats_num);
			session->port[MVX_DIR_INPUT]
				.enc_stats_queue[enc_stats_num] = *stats;
...
int mvx_session_set_fw_profiling(struct mvx_session *session, int val)
{
	if (session->error != 0)
		return session->error;
...
int mvx_session_set_multi_sps_pps(struct mvx_session *session, int val)
{
	if (session->error != 0)
		return session->error;
...
int mvx_session_set_change_pos(struct mvx_session *session, int val)
{
	if (session->error != 0)
		return session->error;
