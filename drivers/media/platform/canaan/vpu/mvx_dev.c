...
static struct mvx_client_session *
register_session(struct mvx_client_ops *client, struct mvx_if_session *isession)
{
	struct mvx_dev_ctx *ctx = client_ops_to_ctx(client);
...
static irqreturn_t irq_top(int irq, void *dev_id)
{
	...
	while (nlsid-- > 0)
		if ((irqve >> nlsid) & 0x1) {
			mvx_hwreg_write_lsid(&ctx->hwreg, nlsid,
					     MVX_HWREG_LIRQVE, 0);
...
static int mvx_dev_probe(struct device *dev, struct resource *iores,
			 struct resource *irqres)
{
	...
	MVX_LOG_PRINT(&mvx_log_dev, MVX_LOG_WARNING,
		      ...
		      mvx_hwreg_read(&ctx->hwreg, MVX_HWREG_NLSID), dev->id);
...
static const struct of_device_id mvx_dev_match_table[] = {
	...
	{ .compatible = "canaan,linlon-v5" },
	{ .compatible = "canaan,linlon-v6" },
	{ .compatible = "canaan,linlon-v7" },
	{ .compatible = "canaan,linlon-v8" },
	{ { 0 } }
};
...
static int mvx_pci_probe(struct pci_dev *pdev, const struct pci_device_id *id)
{
	...
	struct resource irqres = { .start = pdev->irq,
				   .end = pdev->irq,
				   .flags = 0 };
	pdev->dev.id = dev_id++;
...
static struct pci_device_id mvx_pci_device_id[] = {
	{ PCI_DEVICE(MVX_PCI_VENDOR, MVX_PCI_DEVICE) },
	{
		0,
	}
};
...
static struct pci_driver mvx_pci_driver = { .name = MVX_DEV_NAME,
