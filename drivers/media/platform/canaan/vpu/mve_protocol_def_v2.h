...
#define MVE_SET_OPT_INDEX_ENC_EXPOSE_REF_FRAMES \
	(34) /* encode only, disabled by default */
...
#define MVE_SET_OPT_INDEX_ENC_STREAM_BITDEPTH \
	(38)
	/* encode, data.bitdepth,
	 * to set other bitdepth of encoded stream than of input frames
	 */
...
#define MVE_SET_OPT_INDEX_ENC_EXTRA_REFS \
	(52) /* configure number of extra ref buffers */
...
#define MVE_SET_OPT_INDEX_FLUSHLESS_REFBANK \
	(58)
	/* configure AFBC ref bank for individual buffer allocation.
	 * Forced internally for flushless resolution change codecs
	 */
...
#define MVE_OPT_DISABLE_FEATURE_AFBC_LEGACY_REF \
	(0x00000100)
	/* Enables tiled AFBC format in reference buffers.
	 * Ignored for decode AFBC output
	 */
...
#define MVE_OPT_QUANT_TABLE_CHROMA (2)
		...
		struct {
			/* For HEVC, tile_cols must be zero. For VP9,
			 * tile_rows and tile_cols must be powers of 2.
			 */
			uint16_t tile_rows;
			uint16_t tile_cols;
		} tiles;
		...
		struct {
/* Scale factors, and their square roots,
 * for the lambda coefficients used by the encoder,
 * in unsigned Q8 fixed-point format.
 * Default (no scaling) is 1.0 (so 0x0100 in hex).
 */
...
struct mve_buffer_frame_afbc {
	uint32_t plane[2];
						/* Addresses for up to two AFBC planes:
	...
	uint16_t afbc_width_in_superblocks[2];
			  ...
			  * two planes.
			  * For first plane:
			  * (cropx + frame_width) <= superblock_width  * afbc_width...
			  */
...
#define MVE_BUFFER_FRAME_AFBC_STRIDE_SET_BY_MVE \
	(0x00000020)
	/* Decode only: By default, the host should set the afbc_width_in_superblocks.
	 * If the value is zero, or if this bit is set, then the MVE sets an appropriate value.
	 */
};
...
struct mve_buffer_frame {
	...
	/* For matching input buffer with output buffers,
	 * the firmware copies these values between frame buffers and bitstream buffers.
	 */
...
#define VP9SPEC_MAX_SEGMENTS 8
	int8_t qp_delta
		[VP9SPEC_MAX_SEGMENTS]; /* Qp delta to use for each segment map region */
...
#define MVE_BUFFER_PARAM_TYPE_RATE_CONTROL_QP_RANGE_I \
	(25) /* special range for I frames, rate_control_qp_range */
...
#define MVE_BUFFER_PARAM_TYPE_GOP_RESET_LTR_PEROID_DYNAMIC \
	(28) /* reset ltr period dynamiclly */
