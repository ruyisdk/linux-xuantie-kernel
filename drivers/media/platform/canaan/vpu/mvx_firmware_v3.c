...
static int get_region_v3(enum mvx_fw_region region, uint32_t *begin,
			 uint32_t *end)
{
	switch (region) {
...
static int to_mve_profile_v3(unsigned int mvx_profile, uint16_t *mve_profile)
{
	int ret = 0;
	...
	case MVX_PROFILE_H264_HIGH_10:
		*mve_profile =
			MVE_OPT_PROFILE_H264_HIGH; //MVE_OPT_PROFILE_H264_HIGH_10
		break;
	default:
...
int mvx_fw_construct_v3(struct mvx_fw *fw, struct mvx_fw_bin *fw_bin,
