		       ...
		       struct mvx_sched_session *session, struct mvx_lsid *lsid)
{
	...
	MVX_LOG_PRINT(&mvx_log_dev, MVX_LOG_INFO,
		      ...
		      mvx_if_session_to_session(session->isession), lsid->lsid,
	...
	list_for_each_entry_safe(pending, tmp, &sched->pending, pending) {
		...
		if (ret != 0) {
			MVX_LOG_PRINT(
				...
				pending,
				mvx_if_session_to_session(pending->isession));
			break;
		}
				...
				struct mvx_hwreg *hwreg, int ind)
{
	...
	mvx_seq_printf(s, "IRQ MVE", ind, "%d\n",
		       mvx_hwreg_read_lsid(hwreg, lsid->lsid, MVX_HWREG_IRQVE));
}
...
static const struct file_operations sched_fops = { .open = sched_open,
						   .read = seq_read,
						   .llseek = seq_lseek,
...
int mvx_sched_construct(struct mvx_sched *sched, struct device *dev,
			struct mvx_if_ops *if_ops, struct mvx_hwreg *hwreg,
			  ...
			  struct mvx_session *s)
{
	...
	MVX_LOG_PRINT(&mvx_log_dev, MVX_LOG_WARNING,
		      "%p    session=%p, lsid=%d, irqve=%08x, irqhost=%08x", s,
		      mvx_if_session_to_session(session->isession), lsid, irqve,
		      irqhost);
}
			   ...
			   struct mvx_sched_session *session)
{
	...
	MVX_LOG_PRINT(
		...
		s, mvx_hwreg_read(hwreg, MVX_HWREG_JOBQUEUE),
		mvx_hwreg_read(hwreg, MVX_HWREG_CORELSID),
