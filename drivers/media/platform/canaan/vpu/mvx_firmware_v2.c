...
static int get_stride90(enum mvx_format format, uint8_t *nplanes,
 ...
 */
static unsigned int read32n(uint32_t *src, unsigned int offset,
 ...
 */
static int read_message(struct mvx_fw *fw, struct mve_comm_area_host *host,
			struct mve_comm_area_mve *mve, unsigned int *code,
			...
			enum mvx_log_fwif_channel channel)
{
	...
	dma_sync_single_for_cpu(fw->dev, virt_to_phys(mve), MVE_PAGE_SIZE,
				DMA_FROM_DEVICE);
	...
	capacity -=
		DIV_ROUND_UP(sizeof(header) + header.size, sizeof(uint32_t));
	if (capacity < 0) {
		MVX_LOG_PRINT(
	 ...
	 */
	wmb();
	dma_sync_single_for_device(fw->dev, virt_to_phys(&host->out_rpos),
 ...
 * Return: New offset in the circular buffer.
 */
static unsigned int write32n(uint32_t *dst, unsigned int offset,
 ...
 */
static int write_message(struct mvx_fw *fw, struct mve_comm_area_host *host,
			 struct mve_comm_area_mve *mve, unsigned int code,
			 void *data, size_t size,
			 enum mvx_log_fwif_channel channel)
{
	...
	dma_sync_single_for_cpu(fw->dev, virt_to_phys(&mve->in_rpos),
	 ...
	 */
	wmb();
	dma_sync_single_for_device(fw->dev, virt_to_phys(host), MVE_PAGE_SIZE,
				   DMA_TO_DEVICE);
	 ...
	 */
	wmb();
	dma_sync_single_for_device(fw->dev, virt_to_phys(&host->in_wpos),
...
static int get_region_v2(enum mvx_fw_region region, uint32_t *begin,
			 uint32_t *end)
{
	switch (region) {
...
static void convert_buffer_general(struct mvx_fw *fw, enum mvx_direction dir,
				   ...
				   struct mve_buffer_general *g)
{
	...
	if (buf->dir == MVX_DIR_OUTPUT && mvx_is_frame(buf->format)) {
		...
		if (buf->general.header.type ==
		    ...
		    g->header.config_size == sizeof(g->config.ad_stats)) {
			buf->general.config.ad_stats.frame_averages =
				g->config.ad_stats.frame_averages;
			buf->general.config.ad_stats.thumbnail_width =
				g->config.ad_stats.thumbnail_width;
			buf->general.config.ad_stats.thumbnail_height =
				g->config.ad_stats.thumbnail_height;
			buf->general.config.ad_stats.ad_stats_flags =
				g->config.ad_stats.ad_stats_flags;
		}
	}
}

static void convert_buffer_frame(struct mvx_fw *fw, enum mvx_direction dir,
				 struct mvx_fw_msg *msg,
...
static void convert_buffer_bitstream(struct mvx_fw *fw, enum mvx_direction dir,
				     struct mvx_fw_msg *msg,
...
static int convert_buffer_param(struct mvx_fw *fw, struct mvx_fw_msg *msg,
...
static int get_buffer(struct mvx_fw *fw, struct mve_comm_area_host *host,
		      struct mve_comm_area_mve *mve, enum mvx_direction dir,
...
static int get_message_v2(struct mvx_fw *fw, struct mvx_fw_msg *msg)
{
	...
	ret = read_message(fw, fw->msg_host, fw->msg_mve, &code, &fw_msg, &size,
	...
	case MVE_RESPONSE_CODE_EVENT:
		MVX_LOG_PRINT(&mvx_log_if, MVX_LOG_INFO, "EVENT code=%d. %s",
			      ...
			      enum mvx_log_fwif_channel channel)
{
	...
	if ((g.header.type == MVE_BUFFER_GENERAL_TYPE_BLOCK_CONFIGS &&
	     g.header.config_size == sizeof(g.config.block_configs)) ||
	    (g.header.type == MVE_BUFFER_GENERAL_TYPE_AD_STATS &&
	     g.header.config_size == sizeof(g.config.ad_stats))) {
		memcpy(&g.config, &buf->general.config, g.header.config_size);
		ret = write_message(fw, host, mve, MVE_BUFFER_CODE_GENERAL, &g,
				    sizeof(struct mve_buffer_general_hdr) +
					    g.header.config_size,
				    channel);
	}
...
static int put_buffer_frame(struct mvx_fw *fw, struct mve_comm_area_host *host,
			    struct mve_comm_area_mve *mve,
			    struct mvx_fw_msg *msg,
			    enum mvx_log_fwif_channel channel)
{
	...
	if (buf->dir == MVX_DIR_INPUT) {
		f.visible_frame_width =
			(session->visible_width > 0) &&
					(session->visible_width <= buf->width) ?
				session->visible_width :
				buf->width;
		f.visible_frame_height =
			(session->visible_height >
			 0) && (session->visible_height <= buf->height) ?
				session->visible_height :
				buf->height;
	...
	if (buf->dir == MVX_DIR_OUTPUT &&
		...
		f.frame_flags |= ((session->dsl_ratio.hor - 1) << 24 |
				  (session->dsl_ratio.ver - 1) << 17);
	}
	if (buf->flags & MVX_BUFFER_INTERLACE)
		f.frame_flags |= MVE_BUFFER_FRAME_FLAG_INTERLACE;

	f.frame_flags |= (buf->flags & MVX_BUFFER_FRAME_FLAG_ROTATION_MASK) >>
			 8;
	f.frame_flags |= (buf->flags & MVX_BUFFER_FRAME_FLAG_MIRROR_MASK) >> 8;
	f.frame_flags |= (buf->flags & MVX_BUFFER_FRAME_FLAG_SCALING_MASK) >> 8;
	f.frame_flags |= (buf->flags & MVX_BUFFER_FRAME_FLAG_GOP_REST) ?
				 MVE_BUFFER_FRAME_FLAG_RESET_GOP :
				 0;
	f.frame_flags |= (buf->flags & MVX_BUFFER_FRAME_FLAG_LTR_REST) ?
				 MVE_BUFFER_FRAME_FLAG_RESET_LTR_PEROID :
				 0;
	...
	ret = write_message(fw, host, mve, MVE_BUFFER_CODE_FRAME, &f, sizeof(f),
			    channel);
...
static int put_fw_opt(struct mvx_fw *fw, struct mve_request_set_option *opt,
		      size_t size)
{
	int ret;

	ret = write_message(fw, fw->msg_host, fw->msg_mve,
			    MVE_REQUEST_CODE_SET_OPTION, opt,
			    offsetof(typeof(*opt), data) + size,
			    MVX_LOG_FWIF_CHANNEL_MESSAGE);
...
static int put_fw_buf_param(struct mvx_fw *fw, struct mve_buffer_param *param,
			    size_t size)
{
	return write_message(fw, fw->buf_in_host, fw->buf_in_mve,
			     MVE_BUFFER_CODE_PARAM, param,
			     offsetof(typeof(*param), data) + size,
			     MVX_LOG_FWIF_CHANNEL_MESSAGE);
}
...
static int put_message_v2(struct mvx_fw *fw, struct mvx_fw_msg *msg)
{
	...
	case MVX_FW_CODE_STATE_CHANGE: {
		...
		ret = write_message(fw, fw->msg_host, fw->msg_mve, code, NULL,
	...
	case MVX_FW_CODE_SET_OPTION: {
		...
		case MVX_FW_SET_RATE_CONTROL_CHANGE_POS: {
			param.type =
				MVE_BUFFER_PARAM_TYPE_RATE_CONTROL_CHANGE_POS;
		...
		case MVX_FW_SET_RATE_CONTROL_JPEG: {
			...
			opt.data.jpeg_rate_control.change_pos =
				msg->set_option.jpeg_rc.change_pos;
		...
		case MVX_FW_SET_RATE_CONTROL: {
			...
			if (msg->set_option.rate_control.target_bitrate == 0) {
				...
				param.data.rate_control.rate_control_mode =
					msg->set_option.rate_control
						.rate_control_mode;
				param.data.rate_control.target_bitrate =
					msg->set_option.rate_control
						.target_bitrate;
				if (msg->set_option.rate_control
					    .rate_control_mode ==
		...
		case MVX_FW_SET_COLOUR_DESC: {
			...
			case MVX_FW_RANGE_UNSPECIFIED:
				param.data.colour_description.range =
					MVE_BUFFER_PARAM_COLOUR_RANGE_UNSPECIFIED;
				break;
			case MVX_FW_RANGE_LIMITED:
				param.data.colour_description.range =
					MVE_BUFFER_PARAM_COLOUR_RANGE_LIMITED;
				break;
			case MVX_FW_RANGE_FULL:
				param.data.colour_description.range =
					MVE_BUFFER_PARAM_COLOUR_RANGE_FULL;
				break;
			default:
				MVX_LOG_PRINT(
					&mvx_log_if, MVX_LOG_WARNING,
			...
			if (msg->set_option.colour_desc.time_scale != 0 ||
			    msg->set_option.colour_desc.num_units_in_tick !=
				    0) {
				param.data.colour_description.time_scale =
					msg->set_option.colour_desc.time_scale;
				param.data.colour_description.num_units_in_tick =
					msg->set_option.colour_desc
						.num_units_in_tick;
				param.data.colour_description
					.timing_flag_info_present_flag = 1;
			}
			if (msg->set_option.colour_desc.flags &
			    MVX_FW_COLOR_DESC_CONTENT_VALID) {
				param.data.colour_description
					.avg_content_light_level =
					msg->set_option.colour_desc.content
						.luminance_average;
				param.data.colour_description
			...
			if (msg->set_option.colour_desc.flags &
				...
				param.data.colour_description
					.mastering_display_primaries_x[0] =
					msg->set_option.colour_desc.display.r.x;
				param.data.colour_description
					.mastering_display_primaries_x[1] =
					msg->set_option.colour_desc.display.g.x;
				param.data.colour_description
					.mastering_display_primaries_x[2] =
					msg->set_option.colour_desc.display.b.x;
				param.data.colour_description
					.mastering_display_primaries_y[0] =
					msg->set_option.colour_desc.display.r.y;
				param.data.colour_description
					.mastering_display_primaries_y[1] =
				...
				param.data.colour_description
					...
					msg->set_option.colour_desc.display
						.luminance_min;
				param.data.colour_description
		...
		case MVX_FW_SET_VP9_PROB_UPDATE: {
			...
			ret = to_mve_vp9_prob_update(
				msg->set_option.vp9_prob_update, &opt.data.arg);
			if (ret == 0)
				ret = put_fw_opt(fw, &opt,
						 sizeof(opt.data.arg));
		...
		case MVX_FW_SET_HUFF_TABLE: {
			...
			if (msg->set_option.huff_table.type &
				...
				memcpy(opt.data.huffman_table
					       .number_of_huffman_of_code_length,
				       msg->set_option.huff_table
					       .dc_luma_code_lenght,
				       sizeof(msg->set_option.huff_table
						      .dc_luma_code_lenght));
		...
		case MVX_FW_SET_ROI_REGIONS: {
			...
			for (; i < msg->set_option.roi_config.num_roi; i++) {
				param.data.regions.region[i].mbx_left =
					msg->set_option.roi_config.roi[i]
						.mbx_left;
		...
		case MVX_FW_SET_LONG_TERM_REF: {
			...
			if (msg->set_option.ltr.period >= 1 &&
			    msg->set_option.ltr.period <= 255) {
		...
		if (mvx_is_frame(msg->buf->format)) {
			if (msg->buf->dir == MVX_DIR_OUTPUT) {
				if ((msg->buf->flags &
				    ...
				    MVX_BUFFER_FRAME_FLAG_GENERAL) {
					ret = put_buffer_general(fw, host, mve,
								 msg, channel);
					ret = put_buffer_frame(fw, host, mve,
							       msg, channel);
				} else {
					ret = put_buffer_frame(fw, host, mve,
							       msg, channel);
	...
	case MVX_FW_CODE_EOS: {
		...
		if (msg->eos_is_frame != false) {
			...
			struct mve_buffer_bitstream b = {
				...
				.bitstream_flags = MVE_BUFFER_BITSTREAM_FLAG_EOS
			};
			  ...
			  struct mve_rpc_communication_area *rpc_area)
{
	...
	log2_alignment = p->mem_alloc.log2_alignment <= MVE_PAGE_SHIFT ?
				 MVE_PAGE_SHIFT :
	...
	MVX_LOG_PRINT(
		...
		p->mem_alloc.size, p->mem_alloc.max_size, p->mem_alloc.region,
		npages, va);
	...
	if (pages != 0) {
		...
		if (fw->fw_bin->securevideo != false) {
			...
			if (size < p->mem_resize.new_size) {
				...
				ret = mvx_mmu_pages_append_dma_buf(pages,
								   dmabuf);
				if (ret != 0) {
					dma_buf_put(dmabuf);
					goto unlock_mutex;
			 ...
			 struct mve_rpc_communication_area *rpc_area)
{
	...
	if (pages != NULL) {
		hash_del(&pages->node);
		if (MVE_MEM_REGION_PROTECTED_ADDR_BEGIN <=
			    p->mem_free.ve_pointer &&
		    p->mem_free.ve_pointer <
			    MVE_MEM_REGION_PROTECTED_ADDR_END) {
			fw->latest_used_region_protected_pages =
				fw->latest_used_region_protected_pages >
						pages->capacity ?
					fw->latest_used_region_protected_pages -
						pages->capacity :
					0;
		} else if (MVE_MEM_REGION_FRAMEBUF_ADDR_BEGIN <=
				   p->mem_free.ve_pointer &&
			   p->mem_free.ve_pointer <
				   MVE_MEM_REGION_FRAMEBUF_ADDR_END) {
			fw->latest_used_region_outbuf_pages =
				fw->latest_used_region_outbuf_pages >
						pages->capacity ?
					fw->latest_used_region_outbuf_pages -
						pages->capacity :
	...
	MVX_LOG_PRINT(&mvx_log_if, MVX_LOG_INFO, "RPC free memory. va=0x%x.",
		      p->mem_free.ve_pointer);
...
static int handle_rpc_v2(struct mvx_fw *fw)
{
	...
	dma_sync_single_for_cpu(fw->dev, virt_to_phys(rpc_area),
				sizeof(*rpc_area), DMA_FROM_DEVICE);
	...
	if (rpc_area->state == MVE_RPC_STATE_PARAM) {
		...
		case MVE_RPC_FUNCTION_DEBUG_PRINTF: {
			MVX_LOG_PRINT(
				&mvx_log_if, MVX_LOG_INFO, "RPC_PRINT=%s",
		...
		wmb();
		dma_sync_single_for_device(fw->dev, virt_to_phys(rpc_area),
...
static int handle_fw_ram_print_v2(struct mvx_fw *fw)
{
	...
	dma_sync_single_for_cpu(fw->dev, virt_to_phys(rpt_area),
				sizeof(*rpt_area), DMA_FROM_DEVICE);

	wr_cnt = rpt_area->wr_cnt;
	rd_cnt = rpt_area->rd_cnt;
	cnt = (rd_cnt <= wr_cnt) ? wr_cnt - rd_cnt :
				   wr_cnt - rd_cnt + (uint32_t)~0u;

	if (RAM_PRINT_FLAG == rpt_area->flag && RAM_PRINT_BUF_CNT > rpt_area->index && cnt) {
		while (cnt--) {
			rd_idx = rd_cnt % RAM_PRINT_BUF_CNT;
			print_buf = (fw->fw_print_ram + RAM_PRINT_MAX_LEN) +
...
static void unmap_fw_print_ram(struct mvx_fw *fw, void **data,
			       enum mvx_fw_region region)
{
	int ret;
	mvx_mmu_va begin;
	...
	mvx_mmu_free_contiguous_pages(fw->dev, virt_to_phys(*data),
				      MVE_FW_PRINT_RAM_SIZE >> PAGE_SHIFT);

	*data = NULL;
}

static int map_fw_print_ram(struct mvx_fw *fw, void **data,
			    enum mvx_fw_region region)
{
	phys_addr_t page;
	mvx_mmu_va begin;
	...
	page = mvx_mmu_alloc_contiguous_pages(fw->dev, MVE_FW_PRINT_RAM_SIZE >>
							       PAGE_SHIFT);
	if (page == 0)
		return -ENOMEM;
	...
	ret = mvx_mmu_map_pa(fw->mmu, begin, page, MVE_FW_PRINT_RAM_SIZE,
			     MVX_ATTR_SHARED_RW, MVX_ACCESS_READ_WRITE);
	if (ret != 0) {
		mvx_mmu_free_contiguous_pages(
			fw->dev, page, MVE_FW_PRINT_RAM_SIZE >> PAGE_SHIFT);
		return ret;
	}
...
static void print_pair(char *name_in, char *name_out, struct device *device,
		       struct mve_comm_area_host *host,
		       struct mve_comm_area_mve *mve, int ind,
		       struct seq_file *s)
{
	dma_sync_single_for_cpu(device, virt_to_phys(mve), MVE_PAGE_SIZE,
				DMA_FROM_DEVICE);
...
{
	print_pair("Msg host->mve", "Msg host<-mve", fw->dev, fw->msg_host,
		   fw->msg_mve, ind, s);
	print_pair("Inbuf host->mve", "Inbuf host<-mve", fw->dev,
		   fw->buf_in_host, fw->buf_in_mve, ind, s);
	print_pair("Outbuf host->mve", "Outbuf host<-mve", fw->dev,
		   fw->buf_out_host, fw->buf_out_mve, ind, s);
...
static void print_debug_v2(struct mvx_fw *fw)
{
	...
	dma_sync_single_for_cpu(fw->dev, virt_to_phys(msg_mve), MVE_PAGE_SIZE,
				DMA_FROM_DEVICE);

	MVX_LOG_PRINT_SESSION(
		&mvx_log_session_if, MVX_LOG_WARNING, fw->session,
		"Dump message queue. msg={host={out_rpos=%u, in_wpos=%u}, mve={out_wpos=%u, in_rpos=%u}}",
		msg_host->out_rpos, msg_host->in_wpos, msg_mve->out_wpos,
		msg_mve->in_rpos);
	...
	while ((capacity = get_capacity(rpos, wpos)) >= sizeof(header)) {
		...
		MVX_LOG_PRINT_SESSION(
			...
			rpos, wpos, capacity, header.code, header.size);
...
int mvx_fw_send_idle_ack_v2(struct mvx_fw *fw)
{
	...
	ret = write_message(fw, fw->msg_host, fw->msg_mve,
			    MVE_REQUEST_CODE_IDLE_ACK, NULL, 0,
			    MVX_LOG_FWIF_CHANNEL_MESSAGE);
...
int mvx_fw_construct_v2(struct mvx_fw *fw, struct mvx_fw_bin *fw_bin,
			struct mvx_mmu *mmu, struct mvx_session *session,
			struct mvx_client_ops *client_ops,
			struct mvx_client_session *csession,
			unsigned int ncores, unsigned char major,
			unsigned char minor)
{
	int ret;
