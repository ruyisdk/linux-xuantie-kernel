 ...
 */
static bool test_bit_32(int bit, uint32_t *addr)
{
...
static int fw_bin_validate(const struct firmware *fw, struct device *dev)
{
...
static void request_firmware_done(const struct firmware *fw, void *arg)
{
	...
	MVX_LOG_PRINT(
		...
		fw_bin, header->protocol_major, header->protocol_minor,
		header->info_string, header->rasc_jmp,
		fw_bin->nonsecure.text_cnt, fw_bin->nonsecure.bss_cnt,
		fw_bin->nonsecure.sbss_cnt, header->text_length,
 ...
 */
static int hwvercmp(struct mvx_hw_ver *v1, struct mvx_hw_ver *v2)
{
	if (v1->id != v2->id)
		return v1->id - v2->id;
...
static ssize_t hw_ver_show(struct kobject *kobj, struct kobj_attribute *attr,
			   char *buf)
{
	struct mvx_fw_bin *fw_bin = kobj_to_fw_bin(kobj);
	...
	return scnprintf(buf, PAGE_SIZE, "%s-%u-%u\n",
			 hw_id_to_string(hw_ver->id), hw_ver->revision,
			 hw_ver->patch);
}
...
static ssize_t dirty_show(struct kobject *kobj, struct kobj_attribute *attr,
			  char *buf)
{
	struct mvx_fw_bin *fw_bin = kobj_to_fw_bin(kobj);
	int dirty = 0;
...
static struct attribute *fw_bin_attrs[] = { &path_attr.attr, &count_attr.attr,
					    &hw_ver.attr, &dirty_attr.attr,
					    NULL };

static const struct kobj_type fw_bin_ktype = {
					...
					bool securevideo)
{
	...
	case MVX_FORMAT_H264:
		if (dir == MVX_DIR_INPUT)
			fw->size = get_fw_h264dec((unsigned char **)&fw->data);
		else
			fw->size = get_fw_h264enc((unsigned char **)&fw->data);

		break;
	case MVX_FORMAT_HEVC:
		if (dir == MVX_DIR_INPUT)
			fw->size = get_fw_hevcdec((unsigned char **)&fw->data);
		else
			fw->size = get_fw_hevcenc((unsigned char **)&fw->data);

		break;
	case MVX_FORMAT_JPEG:
		if (dir == MVX_DIR_INPUT)
			fw->size = get_fw_jpegdec((unsigned char **)&fw->data);
		else
			fw->size = get_fw_jpegenc((unsigned char **)&fw->data);

		break;
	default:
		MVX_LOG_PRINT(&mvx_log_if, MVX_LOG_ERROR, "wrong format %d.",
			      format);
		break;
	}
 ...
 * into memory.
 */
static struct mvx_fw_bin *
fw_bin_get(struct mvx_fw_cache *cache, enum mvx_format format,
	   enum mvx_direction dir, struct mvx_hw_ver *hw_ver, bool securevideo)
{
...
static struct attribute *cache_attrs[] = { &cache_flush.attr, NULL };
...
static void cache_update(struct mvx_fw_cache *cache)
{
	...
	list_for_each_entry_safe(fw_bin, tmp, &cache->fw_bin_list,
				  cache_head) {
		int ref;
...
int mvx_fw_cache_construct(struct mvx_fw_cache *cache, struct device *dev,
			   ...
			   struct kobject *kobj_parent)
{
	...
	ret = kobject_init_and_add(&cache->kobj, &cache_ktype, kobj_parent,
				   "fw_cache");
	if (ret != 0)
		goto kobj_put;
...
int mvx_fw_cache_get(struct mvx_fw_cache *cache, enum mvx_format format,
		     enum mvx_direction dir, struct mvx_fw_event *event,
		     struct mvx_hw_ver *hw_ver, bool securevideo)
{
	int ret;
	struct mvx_fw_bin *fw_bin;
...
void mvx_fw_cache_put(struct mvx_fw_cache *cache, struct mvx_fw_bin *fw_bin)
{
	int ret;
