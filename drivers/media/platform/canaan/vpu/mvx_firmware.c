 ...
 */
static bool test_bit_32(int bit, const uint32_t *addr)
{
 ...
 */
static int fw_map_core(struct mvx_fw *fw, unsigned int core)
{
	int ret;
	...
	/* Map text segment. */
	ret = mvx_mmu_map_pages(fw->mmu, fw_base + FW_TEXT_BASE_ADDR, fw->text,
				MVX_ATTR_PRIVATE, MVX_ACCESS_EXECUTABLE);
	if (ret != 0)
		return ret;
	...
	for (i = 0; i < header->bss_bitmap_size; i++) {
		if (va >= header->master_rw_start_address &&
			...
			ret = mvx_mmu_map_pa(fw->mmu, fw_base + va,
 ...
 */
static int fw_stat_show(struct seq_file *s, void *v)
{
...
static int fw_stat_open(struct inode *inode, struct file *file)
{
	return single_open(file, fw_stat_show, inode->i_private);
}

static const struct file_operations fw_stat_fops = { .open = fw_stat_open,
						     .read = seq_read,
						     .llseek = seq_lseek,
						     .release =
							     single_release };

static void *rpcmem_seq_start(struct seq_file *s, loff_t *pos)
{
	...
	return mvx_seq_hash_start(fw->dev, fw->rpc_mem, HASH_SIZE(fw->rpc_mem),
				  *pos);
}

static void *rpcmem_seq_next(struct seq_file *s, void *v, loff_t *pos)
{
	struct mvx_fw *fw = s->private;
...
static void rpcmem_seq_stop(struct seq_file *s, void *v)
{
	struct mvx_fw *fw = s->private;
...
static int rpcmem_seq_show(struct seq_file *s, void *v)
{
	struct mvx_seq_hash_it *it = v;
	struct mvx_mmu_pages *pages =
		hlist_entry(it->node, struct mvx_mmu_pages, node);

	if (pages == NULL)
		return 0;

	seq_printf(s, "va = %08x, cap = %08zu, count = %08zu\n", pages->va,
		   pages->capacity, pages->count);

	return 0;
}

static const struct seq_operations rpcmem_seq_ops = { .start = rpcmem_seq_start,
						      .next = rpcmem_seq_next,
						      .stop = rpcmem_seq_stop,
						      .show = rpcmem_seq_show };

static int rpcmem_open(struct inode *inode, struct file *file)
{
	int ret;
	struct seq_file *s;
...
static const struct file_operations rpcmem_fops = { .open = rpcmem_open,
						    .read = seq_read,
						    .llseek = seq_lseek,
						    .release = seq_release };
 ...
 */
static int fw_debugfs_init(struct mvx_fw *fw, struct dentry *parent)
{
	int ret;
...
int mvx_fw_factory(struct mvx_fw *fw, struct mvx_fw_bin *fw_bin,
		   struct mvx_mmu *mmu, struct mvx_session *session,
		   struct mvx_client_ops *client_ops,
		   struct mvx_client_session *csession, unsigned int ncores,
...
int mvx_fw_construct(struct mvx_fw *fw, struct mvx_fw_bin *fw_bin,
		     struct mvx_mmu *mmu, struct mvx_session *session,
		     struct mvx_client_ops *client_ops,
		     struct mvx_client_session *csession, unsigned int ncores)
{
