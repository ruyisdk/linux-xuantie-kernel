...
int mvx_secure_construct(struct mvx_secure *secure, struct device *dev)
{
	secure->dev = dev;
 ...
 * waiting process.
 */
static ssize_t firmware_store(struct kobject *kobj, struct kobj_attribute *attr,
 ...
 * @done:    Firmware load callback routine.
 *
 * Return: Valid pointer on success, else ERR_PTR.
 */
static struct mvx_secure_firmware_priv *
secure_firmware_create(struct mvx_secure *secure, const char *name,
		       unsigned int ncores, void *arg,
		       mvx_secure_firmware_done done)
{
	static struct kobj_attribute attr = __ATTR_WO(firmware);
	static struct attribute *attrs[] = { &attr.attr, NULL };
	static const struct kobj_type secure_ktype = {
	...
	char fw_env[140];
	char *env[4] = { "TYPE=firmware", numcores_env, fw_env, NULL };
	size_t n;
	int ret;
...
int mvx_secure_request_firmware_nowait(struct mvx_secure *secure,
				       const char *name, unsigned int ncores,
				       void *arg, mvx_secure_firmware_done done)
{
	struct mvx_secure_firmware_priv *securefw;
 ...
 * waiting process.
 */
static ssize_t memory_store(struct kobject *kobj, struct kobj_attribute *attr,
			    const char *buf, size_t size)
{
	struct mvx_secure_mem *smem =
		container_of(kobj, struct mvx_secure_mem, kobj);
	...
	MVX_LOG_PRINT(&mvx_log_if, MVX_LOG_INFO,
		      "Secure memory allocated. fd=%d.", *fd);
...
struct dma_buf *mvx_secure_mem_alloc(struct mvx_secure *secure, size_t size)
{
	static struct kobj_attribute attr = __ATTR_WO(memory);
	static struct attribute *attrs[] = { &attr.attr, NULL };
	static const struct kobj_type secure_mem_ktype = {
		.release = secure_mem_release, .sysfs_ops = &kobj_sysfs_ops,
		// .default_attrs = attrs
	};
	struct mvx_secure_mem *smem;
	char size_env[32];
	char *env[3] = { "TYPE=memory", size_env, NULL };
	...
	ret = wait_event_interruptible_timeout(
		smem->wait_queue, smem->dmabuf != NULL, msecs_to_jiffies(1000));
	if (ret == 0) {
		MVX_LOG_PRINT(&mvx_log_if, MVX_LOG_WARNING,
