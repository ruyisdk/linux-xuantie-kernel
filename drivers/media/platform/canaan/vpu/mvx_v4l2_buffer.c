...
static void v4l2_buffer_show(struct mvx_v4l2_buffer *buffer, struct seq_file *s)
{
	...
	for (i = 0; i < vb2->num_planes; ++i) {
		...
		mvx_seq_printf(
			...
			plane->bytesused, plane->length, plane->m.offset,
			plane->m.userptr, plane->m.fd, plane->data_offset);
	}

	ind--;
}

static int buffer_stat_show(struct seq_file *s, void *v)
{
...
static int buffer_stat_open(struct inode *inode, struct file *file)
{
...
static int update_mvx_flags(struct mvx_buffer *buf, struct vb2_buffer *b)
{
	...
	if (mvx_is_afbc(buf->format)) {
		if ((flags & V4L2_BUF_FLAG_MVX_AFBC_TILED_HEADERS) ==
		    V4L2_BUF_FLAG_MVX_AFBC_TILED_HEADERS)
			buf->flags |= MVX_BUFFER_AFBC_TILED_HEADERS;

		if ((flags & V4L2_BUF_FLAG_MVX_AFBC_TILED_BODY) ==
		    V4L2_BUF_FLAG_MVX_AFBC_TILED_BODY)
			buf->flags |= MVX_BUFFER_AFBC_TILED_BODY;

		if ((flags & V4L2_BUF_FLAG_MVX_AFBC_32X8_SUPERBLOCK) ==
		    V4L2_BUF_FLAG_MVX_AFBC_32X8_SUPERBLOCK)
		...
		if (buf->dir == MVX_DIR_OUTPUT) {
			...
			if (flags & V4L2_BUF_FRAME_FLAG_ROTATION_MASK) {
				if ((flags &
				     V4L2_BUF_FRAME_FLAG_ROTATION_MASK) ==
				    V4L2_BUF_FRAME_FLAG_ROTATION_90) {
					buf->flags |=
						MVX_BUFFER_FRAME_FLAG_ROTATION_90;
				} else if ((flags &
					    V4L2_BUF_FRAME_FLAG_ROTATION_MASK) ==
					   V4L2_BUF_FRAME_FLAG_ROTATION_180) {
					buf->flags |=
						MVX_BUFFER_FRAME_FLAG_ROTATION_180;
				} else if ((flags &
					    V4L2_BUF_FRAME_FLAG_ROTATION_MASK) ==
					   V4L2_BUF_FRAME_FLAG_ROTATION_270) {
					buf->flags |=
						MVX_BUFFER_FRAME_FLAG_ROTATION_270;
				}
			}
			if (flags & V4L2_BUF_FRAME_FLAG_SCALING_MASK) {
				if ((flags &
				     V4L2_BUF_FRAME_FLAG_SCALING_MASK) ==
				    V4L2_BUF_FRAME_FLAG_SCALING_2) {
					buf->flags |=
						MVX_BUFFER_FRAME_FLAG_SCALING_2;
				} else if ((flags &
					    V4L2_BUF_FRAME_FLAG_SCALING_MASK) ==
					   V4L2_BUF_FRAME_FLAG_SCALING_4) {
					buf->flags |=
						MVX_BUFFER_FRAME_FLAG_SCALING_4;
				}
			}
			...
			if (flags & V4L2_BUF_FRAME_FLAG_MIRROR_MASK) {
				if ((flags & V4L2_BUF_FRAME_FLAG_MIRROR_MASK) ==
				    V4L2_BUF_FRAME_FLAG_MIRROR_HORI) {
					buf->flags |=
						MVX_BUFFER_FRAME_FLAG_MIRROR_HORI;
				} else if ((flags &
					    V4L2_BUF_FRAME_FLAG_MIRROR_MASK) ==
					   V4L2_BUF_FRAME_FLAG_MIRROR_VERT) {
					buf->flags |=
						MVX_BUFFER_FRAME_FLAG_MIRROR_VERT;
				}
			}
			if (flags & V4L2_BUF_ENCODE_FLAG_ROTATION_MASK) {
				if ((flags &
				     V4L2_BUF_ENCODE_FLAG_ROTATION_MASK) ==
				    V4L2_BUF_ENCODE_FLAG_ROTATION_90) {
					buf->flags |=
						MVX_BUFFER_FRAME_FLAG_ROTATION_90;
				} else if ((flags &
					    V4L2_BUF_ENCODE_FLAG_ROTATION_MASK) ==
					   V4L2_BUF_ENCODE_FLAG_ROTATION_180) {
					buf->flags |=
						MVX_BUFFER_FRAME_FLAG_ROTATION_180;
				} else if ((flags &
					    V4L2_BUF_ENCODE_FLAG_ROTATION_MASK) ==
					   V4L2_BUF_ENCODE_FLAG_ROTATION_270) {
					buf->flags |=
						MVX_BUFFER_FRAME_FLAG_ROTATION_270;
				}
			}
			...
			if ((flags & V4L2_BUF_FLAG_MVX_BUFFER_ROI) ==
			    V4L2_BUF_FLAG_MVX_BUFFER_ROI) {
				buf->flags |= MVX_BUFFER_FRAME_FLAG_ROI;
			}
			if ((flags & V4L2_BUF_FLAG_MVX_BUFFER_CHR) ==
			    V4L2_BUF_FLAG_MVX_BUFFER_CHR) {
				buf->flags |= MVX_BUFFER_FRAME_FLAG_CHR;
			}
			if ((flags & V4L2_BUF_FLAG_MVX_BUFFER_GOP_RESET) ==
			    V4L2_BUF_FLAG_MVX_BUFFER_GOP_RESET) {
...
static int update_vb2_flags(struct mvx_v4l2_buffer *vbuf)
{
	...
	b->flags &= ~(V4L2_BUF_FLAG_ERROR | V4L2_BUF_FLAG_KEYFRAME |
		      V4L2_BUF_FLAG_LAST);
...
int mvx_v4l2_buffer_construct(struct mvx_v4l2_buffer *vbuf,
			      ...
			      enum mvx_direction dir, unsigned int nplanes,
			      struct sg_table **sgt)
{
	int ret;

	ret = mvx_buffer_construct(&vbuf->buf, vsession->ext->dev,
				   &vsession->session.mmu, dir, nplanes, sgt);
	if (ret != 0)
		return ret;
...
int mvx_v4l2_buffer_set(struct mvx_v4l2_buffer *vbuf, struct vb2_buffer *b)
{
	int ret;
