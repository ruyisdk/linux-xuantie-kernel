...
static bool is_alloc(struct mvx_lsid *lsid)
{
	...
	alloc = mvx_hwreg_read_lsid(lsid->hwreg, lsid->lsid, MVX_HWREG_ALLOC);

	return alloc != MVE_ALLOC_FREE;
}

static uint32_t get_core_lsid(uint32_t reg, unsigned int core)
{
	return (reg >> (MVE_CORELSID_LSID_BITS * core)) &
	       MVX_CORELSID_LSID_MASK;
}

static uint32_t get_jobqueue_job(uint32_t reg, unsigned int nr)
{
	return (reg >> (MVE_JOBQUEUE_JOB_BITS * nr)) & MVE_JOBQUEUE_JOB_MASK;
}

static uint32_t set_jobqueue_job(uint32_t reg, unsigned int nr, uint32_t job)
{
...
static uint32_t get_jobqueue_lsid(uint32_t reg, unsigned int nr)
{
	return (reg >> (MVE_JOBQUEUE_JOB_BITS * nr + MVE_JOBQUEUE_LSID_SHIFT)) &
	       MVE_JOBQUEUE_LSID_MASK;
}

static uint32_t set_lsid_ncores(uint32_t reg, unsigned int nr,
				unsigned int lsid, unsigned int ncores)
{
	reg &= ~(MVE_JOBQUEUE_JOB_MASK << (nr * MVE_JOBQUEUE_JOB_BITS));
...
int mvx_lsid_construct(struct mvx_lsid *lsid, struct device *dev,
		       struct mvx_hwreg *hwreg, unsigned int id)
{
	lsid->dev = dev;
	lsid->hwreg = hwreg;
	lsid->session = NULL;
...
int mvx_lsid_map(struct mvx_lsid *lsid, struct mvx_lsid_pcb *pcb)
{
	...
	mvx_hwreg_write_lsid(hwreg, lsid->lsid, MVX_HWREG_CTRL, pcb->ctrl);
...
void mvx_lsid_unmap(struct mvx_lsid *lsid, struct mvx_lsid_pcb *pcb)
{
	struct mvx_hwreg *hwreg = lsid->hwreg;

	if (!is_alloc(lsid)) {
		MVX_LOG_PRINT(&mvx_log_dev, MVX_LOG_WARNING,
			      "LSID was not allocated. lsid=%u.", lsid->lsid);
		return;
	}
	...
	pcb->mmu_ctrl =
		mvx_hwreg_read_lsid(hwreg, lsid->lsid, MVX_HWREG_MMU_CTRL);
	pcb->irqhost =
		mvx_hwreg_read_lsid(hwreg, lsid->lsid, MVX_HWREG_IRQHOST);
...
int mvx_lsid_jobqueue_add(struct mvx_lsid *lsid, unsigned int ncores)
{
	struct mvx_hwreg *hwreg = lsid->hwreg;
	uint32_t jobqueue;
	int i;
