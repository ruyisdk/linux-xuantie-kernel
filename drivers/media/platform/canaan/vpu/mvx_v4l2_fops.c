...
unsigned int mvx_v4l2_poll(struct file *file, struct poll_table_struct *wait)
{
	...
	revents |=
		vb2_poll(&vsession->port[MVX_DIR_INPUT].vb2_queue, file, wait);
	revents |=
		vb2_poll(&vsession->port[MVX_DIR_OUTPUT].vb2_queue, file, wait);

	MVX_SESSION_VERBOSE(
		&vsession->session,
		"v4l2: Poll. events=0x%lx, revents=0x%x, nevents=%d.", events,
		revents, v4l2_event_pending(&vsession->fh));
