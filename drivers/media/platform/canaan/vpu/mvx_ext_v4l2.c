...
int mvx_ext_if_construct(struct mvx_ext_if *ext, struct device *dev,
			 ...
			 struct dentry *parent)
{
	...
	ext->vdev.device_caps = V4L2_CAP_VIDEO_M2M | V4L2_CAP_VIDEO_M2M_MPLANE |
	...
	ret = video_register_device(&ext->vdev,
				    /*VFL_TYPE_SUBDEV*/ VFL_TYPE_VIDEO, -1);

	if (ret != 0) {
		MVX_LOG_PRINT(&mvx_log_if, MVX_LOG_ERROR,
			      "Failed to register video device. ret=%d.", ret);
