 ...
 */
static int get_stride(enum mvx_format format, uint8_t *nplanes,
...
static int map_plane(struct mvx_buffer *buf, mvx_mmu_va begin, mvx_mmu_va end,
		     unsigned int plane)
{
	while (begin < end) {
...
void mvx_buffer_show(struct mvx_buffer *buf, struct seq_file *s)
{
	int i;
	int ind = 0;
	...
	for (i = 0; i < buf->nplanes; ++i) {
		...
		mvx_seq_printf(
			...
			mvx_buffer_va(buf, i), mvx_buffer_size(buf, i),
			plane->stride, plane->filled);
	}

	ind--;
}

int mvx_buffer_construct(struct mvx_buffer *buf, struct device *dev,
			 struct mvx_mmu *mmu, enum mvx_direction dir,
			 unsigned int nplanes, struct sg_table **sgt)
{
	int i;
...
int mvx_buffer_map(struct mvx_buffer *buf, mvx_mmu_va begin, mvx_mmu_va end)
{
	int i;
	int ret = 0;
...
int mvx_buffer_synch(struct mvx_buffer *buf, enum dma_data_direction dir)
{
	int i;
	int ret;
...
int mvx_buffer_filled_set(struct mvx_buffer *buf, unsigned int plane,
			  unsigned int filled, unsigned int offset)
{
	struct mvx_buffer_plane *p = &buf->planes[plane];
...
size_t mvx_buffer_size(struct mvx_buffer *buf, unsigned int plane)
{
	struct mvx_buffer_plane *p = &buf->planes[plane];
...
mvx_mmu_va mvx_buffer_va(struct mvx_buffer *buf, unsigned int plane)
{
	struct mvx_buffer_plane *p = &buf->planes[plane];
...
int mvx_buffer_frame_dim(enum mvx_format format, unsigned int width,
			 ...
			 unsigned int *setting_stride)
{
	...
	for (i = 0; i < *nplanes; i++) {
		...
		unsigned int tmp =
			DIV_ROUND_UP(width * s[i][0], SUBSAMPLE_PIXELS);
		/* Use optimal stride if no special stride was requested. */
		if (i >= __nplanes || stride[i] == 0) {
			if (setting_stride[i]) {
				stride[i] = max(stride[i],
						round_up(tmp, stride_align));
			} else {
		...
		size[i] = DIV_ROUND_UP(height * s[i][1], SUBSAMPLE_PIXELS) *
			  stride[i];
	}
	/* a workaround patch for nv12/nv21/p010 odd height/width output*/
	if (*nplanes == 2 && (width % 2 != 0 || height % 2 != 0)) {
		unsigned int tmp =
			DIV_ROUND_UP(width, SUBSAMPLE_PIXELS) * s[1][0];
		stride[1] = max(stride[1], tmp);
		size[1] = DIV_ROUND_UP(height * s[1][1], SUBSAMPLE_PIXELS) *
			  stride[1];
	}
	return 0;
}

int mvx_buffer_frame_set(struct mvx_buffer *buf, enum mvx_format format,
...
int mvx_buffer_afbc_set(struct mvx_buffer *buf, enum mvx_format format,
			unsigned int width, unsigned int height,
			unsigned int afbc_width, unsigned int size,
			bool interlaced)
{
	int ret;
