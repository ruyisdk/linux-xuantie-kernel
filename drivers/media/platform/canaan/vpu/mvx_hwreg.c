...
static enum mvx_hw_id get_hw_id(void *registers, uint32_t *revision,
				uint32_t *patch)
{
	uint32_t value;
...
static const struct file_operations regs_fops = { .open = regs_open,
						  .read = seq_read,
						  .llseek = seq_lseek,
...
static const struct file_operations lsid_regs_fops = { .open = lsid_regs_open,
						       .read = seq_read,
						       .llseek = seq_lseek,
						       .release =
							       single_release };
...
int mvx_hwreg_construct(struct mvx_hwreg *hwreg, struct device *dev,
...
void mvx_hwreg_write(struct mvx_hwreg *hwreg, enum mvx_hwreg_what what,
		     uint32_t value)
{
	unsigned int offset = get_offset(what);
...
uint32_t mvx_hwreg_read_lsid(struct mvx_hwreg *hwreg, unsigned int lsid,
...
void mvx_hwreg_write_lsid(struct mvx_hwreg *hwreg, unsigned int lsid,
...
enum mvx_hw_id mvx_hwreg_get_hw_id(struct mvx_hwreg *hwreg, uint32_t *revision,
				   uint32_t *patch)
{
