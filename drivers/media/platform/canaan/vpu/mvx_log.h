 ...
 * @_severity:        Severity.
 */
#define MVX_LOG_ENABLED(_lg, _severity) ((_severity) <= (_lg)->severity)
...
#define __MVX_LOG_PRINT(_lg, _severity, _fmt, ...)                           \
	((_lg)->drain->print((_lg)->drain, _severity, (_lg)->tag,            \
			     _fmt " (%s:%d)", __MVX_LOG_N_ARGS(__VA_ARGS__), \
			     ##__VA_ARGS__, mvx_log_strrchr(__FILE__),       \
			     __LINE__))
#else
#define __MVX_LOG_PRINT(_lg, _severity, _fmt, ...)                      \
	((_lg)->drain->print((_lg)->drain, _severity, (_lg)->tag, _fmt, \
			     __MVX_LOG_N_ARGS(__VA_ARGS__), ##__VA_ARGS__))
...
typedef void (*mvx_print_fptr)(struct mvx_log_drain *drain,
			       ...
			       const char *fmt, const unsigned int n_args, ...);
