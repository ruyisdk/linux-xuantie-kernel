...
static void set_format(struct v4l2_pix_format_mplane *pix_mp,
		       ...
		       unsigned int num_planes, unsigned int *sizeimage,
...
static const struct file_operations port_stat_fops = { .open = port_stat_open,
						       .read = seq_read,
						       .llseek = seq_lseek,
						       .release =
							       single_release };

static int port_debugfs_init(struct device *dev, unsigned int i,
...
static struct mvx_v4l2_session *
mvx_session_to_v4l2_session(struct mvx_session *session)
{
			 ...
			 enum mvx_session_event event, void *arg)
{
	...
	MVX_SESSION_INFO(&vsession->session, "Event. event=%d, arg=%p.", event,
			 arg);
	...
	case MVX_SESSION_EVENT_PORT_CHANGED: {
		...
		p->field = port->interlaced ? V4L2_FIELD_SEQ_TB :
					      V4L2_FIELD_NONE;
			       ...
			       struct mvx_ext_if *ctx)
{
	...
	ret = mvx_session_construct(&vsession->session, ctx->dev,
				    ...
				    &vsession->mutex, free_session,
				    handle_event, vsession->dentry);
	if (ret != 0)
		goto remove_dentry;
...
int mvx_v4l2_session_set_seamless_target(
	struct mvx_v4l2_session *vsession,
	struct v4l2_mvx_seamless_target *seamless)
{
	...
	ret = mvx_session_set_seamless_target(&vsession->session,
					      &seamless_target);
	if (ret != 0)
		return ret;
					 ...
					 struct v4l2_mvx_color_conv_coef *coef)
{
	...
	ret = mvx_session_set_color_conversion_ceof(&vsession->session,
						    &conv_coef);
	if (ret != 0)
		return ret;
...
int mvx_v4l2_session_set_rgb_conv_yuv_coef(
	struct mvx_v4l2_session *vsession,
