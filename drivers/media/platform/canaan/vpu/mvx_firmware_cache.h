...
struct mvx_fw_event {
	...
	void (*fw_bin_ready)(struct mvx_fw_bin *fw_bin, void *arg,
			     bool same_thread);
	void *arg;
};
...
int mvx_fw_cache_construct(struct mvx_fw_cache *cache, struct device *dev,
			   struct mvx_secure *secure, struct kobject *kobj);
...
int get_fw_h264dec(unsigned char **data);
int get_fw_h264enc(unsigned char **data);
int get_fw_jpegdec(unsigned char **data);
int get_fw_jpegenc(unsigned char **data);
int get_fw_hevcdec(unsigned char **data);
int get_fw_hevcenc(unsigned char **data);
