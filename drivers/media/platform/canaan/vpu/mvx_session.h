...
#define MVX_SESSION_LOG(severity, session, msg, ...)                       \
	MVX_LOG_PRINT_SESSION(&mvx_log_session_if, severity, session, msg, \
			      ##__VA_ARGS__)
...
struct mvx_session {
	...
	void (*event)(struct mvx_session *session, enum mvx_session_event event,
		      void *arg);
 ...
 */
static inline struct mvx_session *
mvx_if_session_to_session(struct mvx_if_session *session)
{
...
int mvx_session_set_bitrate_control(
 ...
 * Return: 0 in case of success, error code otherwise.
 */
int mvx_session_set_force_chroma_format(struct mvx_session *session, int fmt);
 ...
 * Return: 0 in case of success, error code otherwise.
 */
int mvx_session_set_min_qp(struct mvx_session *session, enum mvx_format format,
			   int qp);
 ...
 * Return: 0 in case of success, error code otherwise.
 */
int mvx_session_set_max_qp(struct mvx_session *session, enum mvx_format format,
			   int qp);
...
int mvx_session_set_mini_frame_height(struct mvx_session *session, int *cnt);
...
int mvx_session_set_color_conversion_ceof(
	struct mvx_session *session, struct mvx_color_conv_coef *conv_coef);
