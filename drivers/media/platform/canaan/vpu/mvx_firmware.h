 ...
 */
enum mvx_fw_state { MVX_FW_STATE_STOPPED, MVX_FW_STATE_RUNNING };
...
struct mvx_buffer_param_region {
	...
	int16_t qp_delta;
	/**< QP delta value. This region will be encoded with qp = qp_default + qp_delta. */
...
#define MVX_MAX_FRAME_CHANGE_RECTANGLES 2
	struct mvx_buffer_param_rectangle
		rectangle[MVX_MAX_FRAME_CHANGE_RECTANGLES];
};
...
#define MVX_MAX_FRAME_OSD_REGION 2
	struct mvx_param_osd_cfg osd_single_cfg
		[MVX_MAX_FRAME_OSD_REGION]; /* include single osd region config and index */
};
...
struct mvx_fw {
	...
	struct {
		...
		void (*unmap_protocol)(struct mvx_fw *fw);
			 ...
			 */
		int (*get_region)(enum mvx_fw_region region, uint32_t *begin,
				  uint32_t *end);
			 ...
			 */
		int (*get_message)(struct mvx_fw *fw, struct mvx_fw_msg *msg);
			 ...
			 * Return: 0 on success, else error code.
			 */
		int (*put_message)(struct mvx_fw *fw, struct mvx_fw_msg *msg);
		...
		int (*handle_fw_ram_print)(struct mvx_fw *fw);
			 ...
			 * Return: 0 on success, else error code.
			 */
		int (*print_stat)(struct mvx_fw *fw, int ind,
				  struct seq_file *s);
