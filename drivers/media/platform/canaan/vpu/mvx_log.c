 ...
 */
static const char *const severity_to_name[] = { "Panic", "Error", "Warning",
						"Info",	 "Debug", "Verbose" };
...
static const char *const severity_to_kern_level[] = {
	KERN_EMERG, KERN_ERR, KERN_WARNING, KERN_NOTICE, KERN_INFO, KERN_DEBUG
};
 ...
 */
static ssize_t readme_read(struct file *file, char __user *user_buffer,
 ...
 * @position:        The current position in the buffer.
 */
static ssize_t group_drain_read(struct file *file, char __user *user_buffer,
				size_t count, loff_t *position)
{
 ...
 * @position:        The current position in the buffer.
 */
static ssize_t drain_ram_read_msg(struct file *file, char __user *user_buffer,
				  size_t count, loff_t *position)
{
 ...
 * @arg:        Extra argument.
 */
static long drain_ram_ioctl(struct file *file, unsigned int cmd,
			    unsigned long arg)
{
...
int mvx_log_construct(struct mvx_log *log, const char *entry_name)
{
	...
	if (!IS_ENABLED(CONFIG_DEBUG_FS)) {
		pr_info("MVX: Debugfs is not enabled. '%s' dir is not created.\n",
			entry_name);
		return 0;
	}
...
static int drain_construct(struct mvx_log_drain *drain, mvx_print_fptr print,
			   mvx_data_fptr data)
{
	drain->print = print;
	drain->data = data;
...
static void drain_dmesg_print(struct mvx_log_drain *drain,
			      enum mvx_log_severity severity, const char *tag,
...
static void drain_dmesg_data(struct mvx_log_drain *drain,
			     enum mvx_log_severity severity, struct iovec *vec,
			     size_t count)
{
	size_t i;
...
int mvx_log_drain_add(struct mvx_log *log, const char *name,
			   ...
			   size_t count)
{
	...
	if (length > drain_ram->buffer_size) {
		pr_err("MVX: Logged data larger than output buffer. length=%zu, buffer_length=%zu.\n",
...
int mvx_log_drain_ram_add(struct mvx_log *log, const char *name,
			  struct mvx_log_drain_ram *drain)
{
	...
	if (!IS_ENABLED(CONFIG_DEBUG_FS)) {
		pr_info("MVX: Debugfs is not enabled. RAM drain dirs are not created.\n");
		return 0;
	}
...
void mvx_log_group_construct(struct mvx_log_group *group, const char *tag,
...
int mvx_log_group_add(struct mvx_log *log, const char *name,
		      struct mvx_log_group *group)
{
	static const struct file_operations group_drain_fops = {
		.read = group_drain_read, .write = group_drain_write
	};
	struct dentry *dentry;
	int ret;
