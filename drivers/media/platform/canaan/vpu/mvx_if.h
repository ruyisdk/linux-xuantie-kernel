 ...
 */
enum mvx_direction { MVX_DIR_INPUT, MVX_DIR_OUTPUT, MVX_DIR_MAX };

/**
 * enum mvx_tristate - Tristate boolean variable.
 */
enum mvx_tristate { MVX_TRI_UNSET = -1, MVX_TRI_TRUE = 0, MVX_TRI_FALSE = 1 };
...
struct mvx_client_ops {
	...
	unsigned int (*get_ncores)(struct mvx_client_ops *client);
		 ...
		 */
	struct mvx_client_session *(*register_session)(
		struct mvx_client_ops *client, struct mvx_if_session *session);
...
struct mvx_if_ops *mvx_if_create(struct device *dev,
				 struct mvx_client_ops *client_ops, void *priv);
