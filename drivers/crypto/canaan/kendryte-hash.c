...
static struct blsegs segment(uint8_t *buf, uint32_t buflen, const uint8_t *in,
				 uint32_t inlen, uint32_t blocksize,
				 uint32_t minlen)
{
	...
	if (buflen != 0) {
		...
		if (nprocblocks * blocksize >= buflen) {
			...
			ret.seg[ret.nsegs++] =
				(struct segstr){ true, buf, proclen };
			start += (proclen - buflen);
			inlen -= (proclen - buflen);
		} else {
			// some data will be remained in the internal buffer
			ret.seg[ret.nsegs++] =
				(struct segstr){ true, buf,
						 nprocblocks * blocksize };
			ret.seg[ret.nsegs++] = (struct segstr){
				false, buf + nprocblocks * blocksize,
	...
	ret.seg[ret.nsegs++] =
		(struct segstr){ false, start + nprocblocks * blocksize,
				 inlen - nprocblocks * blocksize };

	return ret;
}

static int get_key_slot_idx(enum kendryte_key_type_t keytype)
{
	switch (keytype) {
	case SWKEY:
		return 0;
...
//---------------------------------CRYTPO FUNCTION----------------------------------------//
static enum kendryte_status_t crypto_write_dgst(uint8_t *dgst, size_t length)
{
...
static void dma_write_data_block_config(bool head, bool tail, bool dn_intrpt,
...
static void dma_write_rwcfg(struct kendryte_sha256_ctx *ctx, const uint8_t *out,
				const uint8_t *in, uint32_t len)
{
	hash_dma_virt_in = dma_alloc_coherent(ctx->dev->dev, (len + 32),
						  &hash_dma_phys_in, GFP_KERNEL);
	...
	dma_free_coherent(ctx->dev->dev, (len + 32), hash_dma_virt_in,
			  hash_dma_phys_in);
}

static void dma_write_key_config_0(enum kendryte_key_type_t keytype,
				   enum kendryte_algo_type_t algo,
...
static enum kendryte_status_t _ctx_final(struct kendryte_sha256_ctx *hash_ctx,
					 struct kendryte_dgst_st *md)
{
	if (md == NULL)
		return E_INVALID;
...
static enum kendryte_status_t _ctx_update(struct kendryte_sha256_ctx *hash_ctx,
					  struct kendryte_dgst_st *md,
					  ...
					  bool last)
{
	...
	dma_write_data_block_config(hash_ctx->start ? false : true, last, true,
					true, 0);
	dma_write_rwcfg(hash_ctx, NULL, msg, msglen);
	dma_write_key_config_0(hash_ctx->keytype, ALGO_TYPE_HMAC,
				   (hash_ctx->keybits < 512) ? hash_ctx->keybits :
							   512,
				   get_key_slot_idx(hash_ctx->keytype));
	...
	if (!last) {
		...
		hash_ctx->curlen =
			readl(hash_ctx->dev->base + HASH_ALEN_OFFSET);
	}

	return SUCCESS;
}

static enum kendryte_status_t
_sha256_final(struct kendryte_sha256_ctx *hash_ctx, struct kendryte_dgst_st *md)
{
	enum kendryte_status_t check = SUCCESS;

	check = _ctx_update(hash_ctx, md, hash_ctx->buff, hash_ctx->buflen,
				true);
	if (check != SUCCESS)
		goto done;
...
static enum kendryte_status_t
_sha256_update(struct kendryte_sha256_ctx *hash_ctx, const uint8_t *msg,
		   uint32_t msglen)
{
	...
	struct blsegs segs = segment(hash_ctx->buff, hash_ctx->buflen, msg,
					 msglen, hash_ctx->blocklen,
					 hash_ctx->minlen);
	hash_ctx->buflen = 0;
	...
	for (i = 0; i < segs.nsegs; i++) {
		if (segs.seg[i].process) {
			...
			if ((segs.seg[i].addr == hash_ctx->buff) &&
				(hash_ctx->buflen ==
				 0)) { // skip copy what already in the right place
				hash_ctx->buflen += segs.seg[i].len;
			} else {
				// copy into the buffer
				memmove(hash_ctx->buff + hash_ctx->buflen,
...
static enum kendryte_status_t _sha256_init(struct kendryte_sha256_ctx *hash_ctx)
{
	size_t keyaddr;
	uint32_t keybits;
...
static int kendryte_sha256_update(struct ahash_request *req)
{
	...
	check = _sha256_update(ctx, msg, msglen);
	if (check != SUCCESS)
		goto done;
...
static struct ahash_alg
	kendryte_sha256_alg = { .init = kendryte_sha256_init,
				...
				.halg.statesize =
					sizeof(struct kendryte_sha256_ctx),
				.halg.base = {
					.cra_name = "sha256",
