...
/*
 * XuanTie TH1520 nvmem driver support
 *
 * Copyright (C) 2024 Alibaba Group Holding Limited.
 *
 * Author: Wei Liu <lw312886@linux.alibaba.com>
 *
 */
...
#define TH1520_EFUSE_LIT_BLOCK_WIDTH	16
#define TH1520_EFUSE_BIG_BLOCK_WIDTH	32
#define TH1520_EFUSE_LIT_BLOCK_NUM	52
#define TH1520_EFUSE_BIG_BLOCK_NUM	6
#define TH1520_EFUSE_UNIT_SIZE		4

#define RMA_LIFE_CYCLE_PARA		0x1A946F9B
#define RIP_LIFE_CYCLE_PARA		0xEE45E8A7

struct th1520_efuse_priv {
	struct device *dev;
	void __iomem *base;
...
static inline int efuse_status_check(void __iomem *base)
{
	...
	errcode = data & (EFUSE_STA_RD_STATUS_MSK |
			  EFUSE_STA_WR_STATUS_MSK |
			  EFUSE_STA_CMD_ILLEGAL_MSK |
			  EFUSE_STA_KTRANS_ALARM_MSK);
...
static inline int efuse_cmd_start(void __iomem *base,
...
static DEFINE_MUTEX(th1520_efuse_mutex);

static int th1520_efuse_read_start(void __iomem *base, u32 addr,
				   enum con_cmd_type cmd_type)
{
	int ret = 0;
	...
	if (cmd_type == CON_CMD_READ)
		...
		pr_err("invalid efuse read command type\n");
		return -EINVAL;
	}
	...
	ret |= efuse_status_check(base);
	if (ret) {
		pr_err("error occurs while reading the efuse byte at 0x%x\n",
		       addr * TH1520_EFUSE_UNIT_SIZE);
		return ret;
	}
...
static int th1520_efuse_read_word(void __iomem *base, u32 addr, u32 *val)
{
	int ret = 0;
	...
	ret = th1520_efuse_read_start(base, addr, CON_CMD_READ);
	if (ret) {
...
static int th1520_efuse_write_word(void __iomem *base, u32 addr, u32 data,
				   u32 mask)
{
	int ret = 0;
...
exit:
	...
	ret |= efuse_status_check(base);
	if (ret)
		pr_err("error occurs while writing the efuse byte at 0x%x\n",
		       addr * TH1520_EFUSE_UNIT_SIZE);
...
static int th1520_efuse_read(void *context, unsigned int addr, void *data,
			     size_t bytes)
{
	int ret = 0;
	u8 *pdst, *psrc;
	u32 byte_offset, read_count, read_addr, value;
	struct th1520_efuse_priv *priv = context;

	mutex_lock(&th1520_efuse_mutex);
	...
	read_addr = addr / TH1520_EFUSE_UNIT_SIZE;

	/* byte_offset != 0, means not aligned, then read first word */
	if (byte_offset) {
		ret = th1520_efuse_read_word(priv->base, read_addr, &value);
		if (ret) {
			dev_err(priv->dev, "failed to read efuse data\n");
			goto read_end;
		}

		read_count = TH1520_EFUSE_UNIT_SIZE - byte_offset;
	...
	while (bytes >= TH1520_EFUSE_UNIT_SIZE) {
		ret = th1520_efuse_read_word(priv->base, read_addr, &value);
		if (ret) {
			dev_err(priv->dev, "failed to read efuse data\n");
			goto read_end;
		}
		memcpy(pdst, &value, TH1520_EFUSE_UNIT_SIZE);
		bytes -= TH1520_EFUSE_UNIT_SIZE;
		pdst += TH1520_EFUSE_UNIT_SIZE;
		read_addr++;
		/* the hardware will span over a word length automatically */
	}
...
read_end:
	mutex_unlock(&th1520_efuse_mutex);

	return ret;
}

static int th1520_efuse_write(void *context, unsigned int addr, void *data,
			      size_t bytes)
{

	int ret;
	u8 *psrc, *pdst;
	u32 byte_offset, write_addr, write_count, mask, value;
	...
	byte_offset = addr & 0x3;
	psrc = (u8 *)data;
	write_addr = addr / TH1520_EFUSE_UNIT_SIZE;
	...
	if (byte_offset) {
		...
		write_count = TH1520_EFUSE_UNIT_SIZE - byte_offset;
		...
		mask = ~value;
		ret = th1520_efuse_write_word(priv->base, write_addr, value,
					      mask);
		if (ret) {
	...
	while (bytes >= TH1520_EFUSE_UNIT_SIZE) {
		value = 0;
		pdst = (u8 *)&value;
		memcpy(pdst, psrc, TH1520_EFUSE_UNIT_SIZE);
				 ...
				 const char *buf, size_t count)
{
	...
	char *start = (char *)buf;
	struct th1520_efuse_priv *priv = dev_get_drvdata(dev);
	/*
	 * echo types:
	...
	ret = th1520_efuse_write(priv, addr, data, len);
	if (ret < 0) {
		kfree(data);
		return ret;
	}
				...
				struct device_attribute *attr, char *buf)
{
	...
	ret = th1520_efuse_read(priv, addr, data, len);
	if (ret < 0)
		goto out;
			    ...
			    const char *buf, size_t count)
{
	...
	regmap_update_bits(priv->teesys_regs, TEE_SYS_EFUSE_DBG_KEY1_OFF,
			   ENABLE_DFT_FUNC_MASK, ENABLE_DFT_FUNC);

	efuse_permission_magic_config(priv->base, cmd_perm_magic_num,
				      CMD_UPDATE_LC);
...
exit:
	regmap_update_bits(priv->teesys_regs, TEE_SYS_EFUSE_DBG_KEY1_OFF,
			   ENABLE_DFT_FUNC_MASK, DISABLE_DFT_FUNC);
	/* Check status, if there has error, reort and clear status */
			       ...
			       const char *buf, size_t count)
{
	...
	if (!strncmp(life_cycle, "LC_RMA", len)) {
		...
		ret = regmap_read(priv->teesys_regs, TEE_SYS_EFUSE_DBG_KEY1_OFF,
				  &data);
		/* Register from tee system */

		if (ret) {
			dev_err(dev, "failed to read data from DBG_KEY1\n");
			return ret;
		}
		...
		ret = regmap_write(priv->teesys_regs,
				   TEE_SYS_EFUSE_DBG_KEY1_OFF, data);
		if (ret) {
...
static struct attribute *th1520_efuse_sysfs_entries[] = {
...
static const struct attribute_group dev_attr_efuse_sysfs_group = {
	.attrs = th1520_efuse_sysfs_entries,
};

static const struct of_device_id th1520_efuse_of_match[] = {
	{.compatible = "xuantie,th1520-efuse"},
	{ /* sentinel */},
};
MODULE_DEVICE_TABLE(of, th1520_efuse_of_match);

static int __maybe_unused th1520_efuse_runtime_suspend(struct device *dev)
{
	struct th1520_efuse_priv *priv = dev_get_drvdata(dev);
	u32 data;
	int ret;
...
static int __maybe_unused th1520_efuse_runtime_resume(struct device *dev)
{
	struct th1520_efuse_priv *priv = dev_get_drvdata(dev);
	int ret = clk_prepare_enable(priv->clk);

	if (ret) {
		dev_err(dev, "%s: failed to get efuse clk\n", __func__);
		return ret;
	}
...
static int __maybe_unused th1520_efuse_suspend(struct device *dev)
{
	u32 data;
	int ret;
	struct th1520_efuse_priv *priv = dev_get_drvdata(dev);
...
static int __maybe_unused th1520_efuse_resume(struct device *dev)
{
	struct th1520_efuse_priv *priv = dev_get_drvdata(dev);
	int ret = clk_prepare_enable(priv->clk);
...
static int th1520_efuse_probe(struct platform_device *pdev)
{
	int ret;
	struct nvmem_device *nvmem;
	struct nvmem_config config = {};
	struct th1520_efuse_priv *priv;
	...
	if (IS_ERR_OR_NULL(priv->clk)) {
		if (PTR_ERR(priv->clk) != -EPROBE_DEFER)
			dev_err(&pdev->dev, "%s: failed to get efuse clk\n",
				__func__);
	...
	if (ret) {
		dev_err(dev, "%s: failed to enable efuse clk\n", __func__);
		return ret;
	}

	priv->teesys_regs = syscon_regmap_lookup_by_phandle(dev->of_node,
							    "xuantie,secsys");
	if (IS_ERR(priv->teesys_regs)) {
		dev_err(dev, "unable to find secsys registers\n");
	...
	config.name = "th1520-efuse";
	config.read_only = false;
	config.stride = 1;
	config.word_size = 1;
	/* the least read and write unit on the upper level */
	config.reg_read = th1520_efuse_read;
	config.reg_write = th1520_efuse_write;
	config.size = TH1520_EFUSE_LIT_BLOCK_NUM *
				TH1520_EFUSE_LIT_BLOCK_WIDTH +
				TH1520_EFUSE_BIG_BLOCK_NUM *
				TH1520_EFUSE_BIG_BLOCK_WIDTH;
	config.priv = priv;
	config.dev = dev;
	...
	dev_info(dev, "succeed to register th1520 efuse driver\n");

	return 0;
}

static const struct dev_pm_ops efuse_runtime_pm_ops = {
	SET_RUNTIME_PM_OPS(th1520_efuse_runtime_suspend,
			   th1520_efuse_runtime_resume, NULL)
	SET_SYSTEM_SLEEP_PM_OPS(th1520_efuse_suspend, th1520_efuse_resume)
};

static struct platform_driver th1520_efuse_driver = {
	.probe	= th1520_efuse_probe,
	.driver	= {
		.name		= "th1520_efuse",
		.of_match_table	= th1520_efuse_of_match,
		.pm		= &efuse_runtime_pm_ops,
	},
};
module_platform_driver(th1520_efuse_driver);

MODULE_AUTHOR("Wei Liu <lw312886@linux.alibaba.com>");
MODULE_DESCRIPTION("XuanTie TH1520 nvmem driver");
MODULE_LICENSE("GPL");
